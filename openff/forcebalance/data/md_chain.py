...
parser.add_argument("quantities", nargs="+", help="Quantities to calculate")
parser.add_argument(
    "--engine", default="gromacs", help='MD program that we are using; choose "gromacs"'
)
parser.add_argument("--length", type=int, default=1, help="Length of simulation chain")
parser.add_argument("--name", default="sim", help="Default simulation names")
parser.add_argument(
    "--temperature", type=float, default=None, help="Simulation temperature"
)
parser.add_argument("--pressure", type=float, default=None, help="Simulation pressure")
parser.add_argument(
    "--nequil", type=int, default=0, help="Number of steps for equilibration run"
)
parser.add_argument(
    "--nsteps", type=int, default=0, help="Number of steps for production run"
)
...
def main():
    ...
    """
    printcool(
        "ForceBalance simulation using engine: %s" % engname.upper(), color=4, bold=True
    )
    # ----
    ...
    for i in range(args.length):
        ...
        # ----
        EngOpts = OrderedDict(
            [("FF", FF), ("pbc", True), ("coords", coords), ("mol", mol)]
        )
        ...
        # Run the simulation. #
        # =====================#
        MDOpts = OrderedDict([("nsteps", args.nsteps), ("nequil", args.nequil)])
    ...
    lp_dump(
        (
            np.asarray(results["values"]),
            np.asarray(results["errors"]),
            np.asarray(results["grads"]),
        ),
        "md_result.p",
    )
