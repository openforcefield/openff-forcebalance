...
def main():
    ...
    if lipid_snapshots < 2:
        raise Exception(
            "Please set the number of lipid time steps so that you collect at least two snapshots (minimum %i)"
    ...
    G, GDx, GDy, GDz = energy_derivatives(
        Lipid, FF, mvals, h, pgrad, len(Energies), AGrad, dipole=True
    )
    ...
    if FDCheck:
        ...
        GRho1 = property_derivatives(
            Lipid, FF, mvals, h, pgrad, kT, calc_rho, {"r_": Rhos}
        )
    ...
    if FDCheck:
        GAlpha_fd = property_derivatives(
            Lipid, FF, mvals, h, pgrad, kT, calc_alpha, {"h_": H, "v_": V}
        )
        Sep = printcool("Numerical Derivative:")
    ...
    if FDCheck:
        GKappa_fd = property_derivatives(
            Lipid, FF, mvals, h, pgrad, kT, calc_kappa, {"v_": V}
        )
        Sep = printcool("Numerical Derivative:")
    ...
    if FDCheck:
        GEps0_fd = property_derivatives(
            Lipid, FF, mvals, h, pgrad, kT, calc_eps0, {"d_": Dips, "v_": V}
        )
        Sep = printcool("Numerical Derivative:")
    ...
    GLKappa2 = (al_avg / al_var**2) * (
        covde(Als_m2**2) - (2 * al_avg * covde(Als_m2))
    )
    GLKappa = (1e3 * 2 * kbT / 128) * (GLKappa1 - GLKappa2)
    FF.print_map(vals=GLKappa)
    if FDCheck:
        GLKappa_fd = property_derivatives(
            Lipid, FF, mvals, h, pgrad, kT, calc_lkappa, {"a_": Als_m2}
        )
        Sep = printcool("Numerical Derivative:")
        FF.print_map(vals=GLKappa_fd)
        Sep = printcool("Difference (Absolute, Fractional):")
        absfrac = [
            f"{i - j: .4e}  {(i - j) / j: .4e}" for i, j in zip(GLKappa, GLKappa_fd)
        ]
        FF.print_map(vals=absfrac)
