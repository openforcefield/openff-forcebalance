...
from openff.forcebalance.finite_difference import f1d2p, f1d7p, f12d3p, fdwrap, in_fd
...
parser.add_argument(
    "engine", help='MD program that we are using; choose "openmm", "amber" or "gromacs"'
)
...
if engname == "openmm":
    ...
    raise Exception(
        "OpenMM, SMIRNOFF/OpenMM, GROMACS, and AMBER are supported at this time."
    )
...
def bzavg(obs, boltz):
    ...
    if obs.ndim == 2:
        if obs.shape[0] == len(boltz) and obs.shape[1] == len(boltz):
            raise Exception(
                "Error - both dimensions have length equal to number of snapshots, now confused!"
            )
...
def PrintEDA(EDA, N):
    ...
    for key, val in EDA.items():
        ...
        if val_err == 0.0:
            continue
        PrintDict[key] = "{: 12.4f} +- {:10.4f} [ {: 9.4f} +- {:7.4f} ]".format(
            val_avg, val_err, val_avg / N, val_err / N
        )
    printcool_dictionary(
        PrintDict, "Energy Component Analysis, Mean +- Stderr [Per Molecule] (kJ/mol)"
    )
...
def property_derivatives(
    ...
    E0 = ED0[:, 0]
    D0 = ED0[:, 1:]
    property_driver(None, **property_kwargs)
    ...
    for i in pgrad:
        ...
        if InfoContent / len(E0) < 0.1:
            logger.warn(
                "Warning: Effective number of snapshots: % .1f (out of %i)\n"
                % (InfoContent, len(E0))
            )
...
def main():
    ...
    printcool(
        "ForceBalance condensed phase simulation using engine: %s" % engname.upper(),
        color=4,
        bold=True,
    )
    ...
    logger.info(
        "For the condensed phase system, I will collect %i snapshots spaced apart by %i x %.3f fs time steps\n"
        % (liquid_snapshots, liquid_iframes, liquid_timestep)
    )
    if liquid_snapshots < 2:
        raise Exception(
            "Please set the number of liquid time steps so that you collect at least two snapshots (minimum %i)"
            % (2000 * int(liquid_intvl / liquid_timestep))
        )
    logger.info(
        "For the gas phase system, I will collect %i snapshots spaced apart by %i x %.3f fs time steps\n"
        % (gas_snapshots, gas_iframes, gas_timestep)
    )
    if gas_snapshots < 2:
        raise Exception(
            "Please set the number of gas time steps so that you collect at least two snapshots (minimum %i)"
    ...
    # ----
    EngOpts = OrderedDict()
    EngOpts["liquid"] = OrderedDict(
        [("coords", liquid_fnm), ("mol", ML), ("pbc", True)]
    )
    ...
    if engname in ["openmm", "smirnoff"]:
        ...
        if force_cuda:
            try:
                ...
                raise RuntimeError("Forcing failure because CUDA platform unavailable")
        ...
        if engname == "smirnoff":
            if not TgtOptions["liquid_coords"].endswith(".pdb"):
                logger.error(
                    "With SMIRNOFF engine, please pass a .pdb file to liquid_coords."
                )
                raise RuntimeError
            ...
            if not TgtOptions["gas_coords"].endswith(".pdb"):
                logger.error(
                    "With SMIRNOFF engine, please pass a .pdb file to gas_coords."
                )
                raise RuntimeError
        ...
        if rpmd_beads > 0:
            raise RuntimeError("Gromacs cannot handle RPMD.")
        if mts:
        ...
        if force_cuda:
            logger.warn("force_cuda option has no effect on Amber engine.")
        if rpmd_beads > 0:
            raise RuntimeError("AMBER cannot handle RPMD.")
        if mts:
            logger.warn("Amber not configured for multiple timestep integrator.")
    ...
    # Set up MD options
    MDOpts = OrderedDict()
    MDOpts["liquid"] = OrderedDict(
        [
            ("nsteps", liquid_nsteps),
            ("timestep", liquid_timestep),
            ("temperature", temperature),
            ("pressure", pressure),
            ("nequil", liquid_nequil),
            ("minimize", minimize),
    ...
    MDOpts["gas"] = OrderedDict(
        [
            ("nsteps", gas_nsteps),
            ("timestep", gas_timestep),
            ("temperature", temperature),
            ("nsave", int(1000 * gas_intvl / gas_timestep)),
            ("nequil", gas_nequil),
            ("minimize", minimize),
            ("threads", 1),
            ("mts", mts),
    ...
    if hasattr(Liquid, "freeze_atoms"):
        logger.info(
            "Warning: freeze_atoms may result in incorrect system mass and incorrect density calculation\n"
        )
    ...
    click()
    G, GDx, GDy, GDz = energy_derivatives(
        Liquid, FF, mvals, h, pgrad, len(Energies), AGrad, dipole=True
    )
    logger.info("Condensed phase energy derivatives took %.3f seconds\n" % click())
    click()
    printcool("Gas phase energy derivatives", color=4, bold=True)
    mG, _, __, ___ = energy_derivatives(
        Gas, FF, mvals, h, pgrad, len(mEnergies), AGrad, dipole=False
    )
    ...
    Sep = printcool(
        "Density: {: .4f} +- {: .4f} kg/m^3\nAnalytic Derivative:".format(
            Rho_avg, Rho_err
        )
    )
    ...
    if FDCheck:
        ...
        GRho1 = property_derivatives(
            Liquid, FF, mvals, h, pgrad, kT, calc_rho, {"r_": Rhos}
        )
        FF.print_map(vals=GRho1)
        Sep = printcool("Difference (Absolute, Fractional):")
        absfrac = [f"{i - j: .4e}  {(i - j) / j: .4e}" for i, j in zip(GRho, GRho1)]
    ...
    logger.info(
        "Liquid enthalpy: % .4f kJ/mol, stdev % .4f ; (% .4f from energy, % .4f from pV)\n"
    ...
    Hvap_err = np.sqrt(
        Ene_err**2 / NMol**2 + mEne_err**2 + pV_err**2 / NMol**2
    )
    ...
    GHvap -= mBeta * (
        flat(np.dot(mG, col(mEnergies))) / L - mEne_avg * np.mean(mG, axis=1)
    )
    GHvap *= -1
    GHvap -= (
        mBeta * (flat(np.dot(G, col(pV))) / L - np.mean(pV) * np.mean(G, axis=1)) / NMol
    )

    Sep = printcool(
        "Enthalpy of Vaporization: {: .4f} +- {:.4f} kJ/mol\nAnalytic Derivative:".format(
            Hvap_avg, Hvap_err
        )
    )
    ...
    def calc_alpha(b=None, **kwargs):
        ...
        return (
            1
            / (kT * T)
            * (bzavg(h_ * v_, b) - bzavg(h_, b) * bzavg(v_, b))
            / bzavg(v_, b)
        )
    ...
    Alpha_err = np.std(Alphaboot) * max(
        [np.sqrt(statisticalInefficiency(V)), np.sqrt(statisticalInefficiency(H))]
    )
    ...
    if FDCheck:
        GAlpha_fd = property_derivatives(
            Liquid, FF, mvals, h, pgrad, kT, calc_alpha, {"h_": H, "v_": V}
        )
        Sep = printcool("Numerical Derivative:")
        FF.print_map(vals=GAlpha_fd)
        Sep = printcool("Difference (Absolute, Fractional):")
        absfrac = [
            f"{i - j: .4e}  {(i - j) / j: .4e}" for i, j in zip(GAlpha, GAlpha_fd)
        ]
        FF.print_map(vals=absfrac)
    ...
    if FDCheck:
        GKappa_fd = property_derivatives(
            Liquid, FF, mvals, h, pgrad, kT, calc_kappa, {"v_": V}
        )
        Sep = printcool("Numerical Derivative:")
        FF.print_map(vals=GKappa_fd)
        Sep = printcool("Difference (Absolute, Fractional):")
        absfrac = [
            f"{i - j: .4e}  {(i - j) / j: .4e}" for i, j in zip(GKappa, GKappa_fd)
        ]
        FF.print_map(vals=absfrac)
    ...
    if FDCheck:
        GCp_fd = property_derivatives(
            Liquid, FF, mvals, h, pgrad, kT, calc_cp, {"h_": H}
        )
        Sep = printcool("Numerical Derivative:")
        FF.print_map(vals=GCp_fd)
        Sep = printcool("Difference (Absolute, Fractional):")
        absfrac = [f"{i - j: .4e}  {(i - j) / j: .4e}" for i, j in zip(GCp, GCp_fd)]
        FF.print_map(vals=absfrac)
    ...
    GD2 = 2 * (
        flat(np.dot(GDx, col(Dx))) / L - avg(Dx) * (np.mean(GDx, axis=1))
    ) - Beta * (covde(Dx**2) - 2 * avg(Dx) * covde(Dx))
    GD2 += 2 * (
        flat(np.dot(GDy, col(Dy))) / L - avg(Dy) * (np.mean(GDy, axis=1))
    ) - Beta * (covde(Dy**2) - 2 * avg(Dy) * covde(Dy))
    GD2 += 2 * (
        flat(np.dot(GDz, col(Dz))) / L - avg(Dz) * (np.mean(GDz, axis=1))
    ...
    if FDCheck:
        GEps0_fd = property_derivatives(
            Liquid, FF, mvals, h, pgrad, kT, calc_eps0, {"d_": Dips, "v_": V}
        )
        Sep = printcool("Numerical Derivative:")
        FF.print_map(vals=GEps0_fd)
        Sep = printcool("Difference (Absolute, Fractional):")
        absfrac = [f"{i - j: .4e}  {(i - j) / j: .4e}" for i, j in zip(GEps0, GEps0_fd)]
        FF.print_map(vals=absfrac)
