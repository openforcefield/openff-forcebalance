...
parser = argparse.ArgumentParser(
    "nvt simulation tool for computing surface tension and derivatives.",
...
def main():
    ...
    printcool(
        "ForceBalance condensed phase NVT simulation using engine: %s"
        % engname.upper(),
        color=4,
        bold=True,
    )
    ...
    TgtOptions.get("n_mcbarostat", 25)
    TgtOptions.get("anisotropic_box", 0)
    ...
    box_vectors = np.array(
        Liquid.xyz_omms[0][1] / nanometer
    )  # obtain original box vectors from first frame
    delta_S = (
        np.sqrt(np.sum(np.cross(box_vectors[0], box_vectors[1]) ** 2))
        * perturb_proportion
        * 2
    )  # unit: nm^2. *2 for 2 surfaces
    # perturb xy area +
    click()
    ...
    if AGrad:
        G_plus, _, _, _ = energy_derivatives(
            Liquid, FF, mvals, h, pgrad, len(Potentials), AGrad, dipole=False
        )
        logger.info(
    ...
    if AGrad:
        G_minus, _, _, _ = energy_derivatives(
            Liquid, FF, mvals, h, pgrad, len(Potentials), AGrad, dipole=False
        )
        logger.info(
            "Calculation of energy gradients for perturbed box- took %.3f seconds\n"
    ...
    surf_ten = prefactor * (
        np.log(np.mean(exp_dE_plus)) - np.log(np.mean(exp_dE_minus))
    )
    ...
    for i in range(numboots):
        ...
        surf_ten_boots[i] = prefactor * (
            np.log(np.mean(boots_exp_dE_plus)) - np.log(np.mean(boots_exp_dE_minus))
        )
    ...
    printcool(
        "Surface Tension:       {: .4f} +- {:.4f} mJ m^-2".format(
    ...
    if AGrad:
        ...
        for param_i in range(n_params):
            ...
            G_surf_ten[param_i] = prefactor * (
                plus_numer / plus_denom - minus_numer / minus_denom
            )
