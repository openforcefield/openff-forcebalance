...
import openff.forcebalance.parser
...
class TestParser(ForceBalanceTestCase):
    def test_parse_inputs_returns_tuple(self):
        ...
        assert isinstance(output[0], dict), (
            "\nExpected parse_inputs()[0] to be an options dictionary, got a %s instead"
            % type(output).__name__
        )
        assert isinstance(output[1], list), (
            "\nExpected parse_inputs()[1] to be a target list, got a %s instead"
            % type(output[1]).__name__
        )
        assert isinstance(output[1][0], dict), (
            "\nExpected parse_inputs()[1][0] to be a target dictionary, got a %s instead"
            % type(output[1]).__name__
        )
