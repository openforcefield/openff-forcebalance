...
def fitting_cd(
    ...
    source = pathlib.Path(__file__).parent.parent.parent / "fitting-tests"  / directory_name
    assert source.is_dir()
    original_directory = os.getcwd()

    try:
        with tempfile.TemporaryDirectory() as new_directory:
            os.chdir(shutil.copytree(source.as_posix(), new_directory, dirs_exist_ok=True))
