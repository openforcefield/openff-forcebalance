...
import openff.forcebalance.molecule
...
class TestPDBMolecule(ForceBalanceTestCase):
    ...
    def teardown_method(self):
        os.system(
            "rm -rf {name}.xyz {name}.gro {name}.arc".format(name=self.source[:-4])
        )
    ...
    def test_xyz_conversion(self):
        ...
        self.logger.debug(
            "Checking that conversion has not changed molecule spatial coordinates\n"
        )
        assert (self.molecule.Data["xyzs"][0] == molecule1.Data["xyzs"][0]).all(), (
            "Conversion from pdb to xyz yields "
    ...
    def test_gro_conversion(self):
        ...
        np.testing.assert_allclose(
            self.molecule.Data["xyzs"][0],
            molecule1.Data["xyzs"][0],
            rtol=0,
            atol=0.001,
            err_msg=msg,
        )
    ...
    def test_arc_conversion(self):
        ...
        self.molecule.Data["tinkersuf"] = [""] * len(
            self.molecule.Data["resname"]
        )  # suppress topology warning
...
class TestLipidGRO(ForceBalanceTestCase):
    ...
    def test_measure_dihedrals(self):
        ...
        np.testing.assert_allclose(
            EXPECTED_DIHEDRAL_RESULTS, result, atol=0.001, err_msg=msg
        )
    ...
    def test_lipid_molecules(self):
        ...
        assert (
            len(self.molecule.molecules) == 3783
...
class TestWaterPDB(ForceBalanceTestCase):
    ...
    def test_water_molecules(self):
        ...
        assert (
            len(self.molecule.molecules) == 500
...
class TestGalbPNPMol2(ForceBalanceTestCase):
    ...
    def test_read_galb(self):
        """Check for the correct number of bonds in a simple molecule"""
        self.logger.debug("\nTrying to read alanine dipeptide conformation... ")
        assert self.molecule.resname == 14 * ["PNP"] + 22 * [
            "0LB"
        ], "Incorrect residue names"
        assert self.molecule.elem == [
            "O",
            "N",
            "O",
            "C",
