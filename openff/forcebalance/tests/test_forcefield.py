...
import openff.forcebalance.forcefield as forcefield
...
class FFTests:
    ...
    def test_FF_yields_consistent_results(self):
        ...
        assert forcefield.FF(self.options) == forcefield.FF(
            self.options
    ...
    def test_make_function_return_value(self):
        ...
        self.logger.debug(
            "Running forcefield.make() with zero vector should not change pvals... "
        )
        ...
        self.logger.debug(
            "make() should return different values when passed in nonzero pval matrix... "
        )
        ...
        assert not (
            pvals == new_pvals
        ).all(), "make() returned unchanged pvals even when given nonzero matrix"
        self.logger.debug("ok\n")
        ...
        np.testing.assert_array_almost_equal(
            np.ones(self.ff.np), new_pvals, decimal=5, err_msg=msg
        )
        self.logger.debug("ok\n")
    ...
    def test_make_function_output(self):
        ...
        os.rename(
            self.ff.fnms[0], self.options["ffdir"] + "/test_zeros." + self.filetype
        )
        ...
        os.rename(
            self.ff.fnms[0], self.options["ffdir"] + "/test_ones." + self.filetype
        )
