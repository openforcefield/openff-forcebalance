...
EXPECTED_ETHANOL_RESULTS = array(
    [
        1.2286e-01,
        8.3624e-01,
        1.0014e-01,
...
EXPECTED_OPENFF_TORSIONPROFILE_RESULTS = array(
    [
        -9.4238e-02,
        7.3350e-03,
        -7.9467e-05,
    ...
    def teardown_method(self):
        for fnm in [self.input_file.replace(".in", ".sav")]:
            if os.path.exists(fnm):
                os.remove(fnm)
        for dnm in [
            self.input_file.replace(".in", ".bak"),
            self.input_file.replace(".in", ".tmp"),
            "result",
        ]:
    ...
    def run_optimizer(self, check_result=True, check_iter=True, use_pvals=False):
        ...
        if check_result:
            msg = (
                ...
                "If this seems reasonable, update %s in test_system.py with these values"
                % self.expected_results_name
            )
            np.testing.assert_allclose(
                self.expected_results, result, atol=self.absolute_tolerance, err_msg=msg
            )
        if check_iter:
            # Fail if calculation takes longer than previously to converge
            assert ITERATIONS_TO_CONVERGE >= Counter(), (
...
class TestEvaluatorBromineStudy(ForceBalanceSystemTest):
    def setup_method(self, method):
        ...
        cwd = os.path.dirname(os.path.realpath(__file__))
        os.chdir(
            os.path.join(cwd, "..", "..", "studies", "003d_evaluator_liquid_bromine")
        )
    ...
    def test_bromine_study(self):
        ...
        np.testing.assert_allclose(
            EXPECTED_EVALUATOR_BROMINE_OBJECTIVE, X, atol=120, err_msg=msgX
        )
        ...
        np.testing.assert_allclose(
            EXPECTED_EVALUATOR_BROMINE_GRADIENT, G, atol=4000, err_msg=msgG
        )
