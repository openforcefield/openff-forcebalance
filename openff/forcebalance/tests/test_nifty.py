...
class TestNifty(ForceBalanceTestCase):
    ...
    def test_nifty_functions(self):
        ...
        Y = (1, 0, -1)
        result = get_least_squares(X, Y)[0]
        np.testing.assert_almost_equal(result[0], 1.0 / 3.0)
        np.testing.assert_almost_equal(result[1], -1.0 / 3.0)
    ...
    def test_work_queue_functions(self):
        ...
        assert type(wq) is work_queue.WorkQueue, (
            "Expected getWorkQueue() to return a "
        ...
        if worker_program != "":
            ...
            worker = subprocess.Popen(
                [
                    os.path.join(worker_program, "work_queue_worker"),
                    "localhost",
                    str(wq.port),
                ],
            ...
            assert (
                wq.stats.total_tasks_complete == 1
            ), "Expected queue to have a task completed"
        else:
