...
from openff.forcebalance.finite_difference import f1d5p, f1d7p, fdwrap
...
class Optimizer(BaseClass):
    ...
    def __init__(self, options, Objective, FF):
        ...
        self.OptTab = {
            "NEWTONRAPHSON": self.NewtonRaphson,
    ...
    def recover(self):
        ...
        if len({T.maxrd() for T in self.Objective.Targets}) == 1 and any(
            ...
            if T.maxrd() == maxrd:
                ...
                if os.path.exists(os.path.join(T.absrd(), "mvals.txt")):
                    ...
                    if len(tmvals) > 0 and np.max(np.abs(tmvals - self.mvals0) > 1e-4):
                        warn_press_key(
                            "mvals.txt in {} does not match loaded parameters.\nSave file : {}\\Parameters : {}\n".format(
    ...
    def Run(self):
        ...
        if self.jobtype.lower() == "precondition":
        ...
        while 1:
            ...
            if ITERATION > self.iterinit:
                ...
                try:
                    ...
                    logger.warning(
                        "Warning: Step size of zero detected (i.e. wrong direction). "
                ...
                if X > (X_prev + max(self.err_tol, self.convergence_objective)):
                    ...
                    self.set_goodstep(0)
                    print_progress(
                        ITERATION, nxk, ndx, ngd, "\x1b[91m", X, X - X_prev, Quality
                    )
                    xk = xk_prev.copy()
                    trust = max(ndx * (1.0 / (1 + self.adapt_fac)), self.mintrust)
                    ...
                    if Quality <= ThreLQ and self.trust0 > 0:
                        ...
                        trust += (
                            self.adapt_fac
                            * trust
                            * np.exp(-1 * self.adapt_damp * (trust / self.trust0 - 1))
                        )
                        if trust > curr_trust:
                    ...
                    if b_BFGS:
                        ...
                        Mat2 = (np.dot(np.dot(Hnew, Dx), np.dot(Hnew, Dx).T)) / (
                            multi_dot([Dx.T, Hnew, Dx])
                        )[0, 0]
                        Hnew += Mat1 - Mat2
                        H = Hnew.copy()
                        data["H"] = H.copy()
            ...
            if self.goodstep:
                print_progress(
                    ITERATION, nxk, ndx, ngd, color, X, dX2_sign * stdfront, Quality
                )
            # ================================#
            ...
            if (
                self.uncert
                or (self.converge_lowq or Quality > ThreLQ)
                or (ITERATION == self.iterinit and self.iterinit > 0)
            ):
                ...
                if (
                    ndx < self.convergence_step
                    and ndx >= 0.0
                    and ITERATION > self.iterinit
                ):
                    logger.info(
            ...
            if sum(criteria_satisfied.values()) >= self.criteria:
                break
            # ================================#
            ...
            if self.print_vals:
                ...
                self.FF.print_map(
                    vals=[
                        "{: .4e} {} {:.4e} = {: .4e}".format(
                            xk_prev[i], "+" if dx[i] >= 0 else "-", abs(dx[i]), xk[i]
                        )
                ...
                self.FF.print_map(
                    vals=[
                        "{: .4e} {} {:.4e} = {: .4e}".format(
                            pk_prev[i], "+" if dp[i] >= 0 else "-", abs(dp[i]), pk[i]
                        )
    ...
    def step(self, xk, data, trust):
        ...
        if self.bhyp:
            ...
            def hyper_solver(L):
                ...
                Opt1 = optimize.fmin_l_bfgs_b(
                    HYP.compute_val,
                    dx0,
                    fprime=HYP.compute_grad,
                    m=30,
                    factr=1e7,
                    pgtol=1e-4,
                    iprint=-1,
                ...
                for (
                    ...
                    self.excision
                ):  # Reinsert deleted coordinates - don't take a step in those directions
        ...
        def search_fun(L):
            ...
            Result = (
                self.Objective.Full(
                    xk_, 0, verbose=False, customdir="micro_%02i" % search_fun.micro
                )["X"]
                - data["X"]
            )
        ...
        if self.trust0 > 0:  # This is the trust region code.
            ...
            if dxnorm > trust:
                ...
                LOpt = optimize.brent(
                    trust_fun, brack=(self.lmg, self.lmg * 4), tol=1e-6
                )
            ...
            if dxnorm > trust:
                LOpt = optimize.brent(
                    trust_fun, brack=(self.lmg, self.lmg * 4), tol=1e-4
                )
                dx, expect = solver(LOpt)
                dxnorm = np.linalg.norm(dx)
            else:
            ...
            bump = False
            search_fun.micro = 0
            Result = optimize.brent(
                search_fun, brack=(LOpt, LOpt * 4), tol=self.search_tol, full_output=1
            )
            if Result[1] > 0:
    ...
    def ScipyOptimizer(self, Algorithm="None"):
                ...
                def print_results(color, newline=True):
                    ...
                    if Order:
                        logger.info(
                            ...
                            % (
                                fout.evals,
                                np.linalg.norm(mvals),
                                np.linalg.norm(mvals - self.xk_prev),
                                np.linalg.norm(G),
                                color,
                                X,
                                X - self.x_prev,
        ...
        if Algorithm == "powell":
            ...
            return optimize.fmin_powell(
                ...
                self.mvals0,
                ftol=self.convergence_objective,
            ...
            xmin, Jmin, T, feval, iters, accept, status = optimize.anneal(
                xwrap(self.Objective.Full),
                self.mvals0,
                lower=self.mvals0 - 1 * self.trust0 * np.ones(self.np),
                upper=self.mvals0 + self.trust0 * np.ones(self.np),
                schedule="boltzmann",
                full_output=True,
            )
            scodes = {
            ...
            return optimize.fmin_cg(
                ...
                self.mvals0,
                fprime=gwrap(self.Objective.Full),
            ...
            Result = optimize.fmin_tnc(
                ...
                maxfun=self.maxstep,
                ftol=self.convergence_objective,
                pgtol=self.convergence_gradient,
                xtol=self.convergence_objective,
            )
            return Result.x
        elif Algorithm == "ncg":
            printcool(
            ...
            Result = optimize.fmin_ncg(
                xwrap(self.Objective.Full, callback=False),
                self.mvals0,
                fprime=gwrap(self.Objective.Full, callback=False),
                fhess=hwrap(self.Objective.Full),
                avextol=self.convergence_objective,
                maxiter=self.maxstep,
                disp=True,
            )
            return Result
    ...
    def GeneticAlgorithm(self):
        ...
        def generate_fresh(rows, cols):
            ...
            for i in range(rows):
                new_guys[i, int(cols * np.random.random())] = (
                    self.trust0 * np.random.randn()
                )
            return new_guys
        ...
        def initial_generation():
            return np.vstack(
                (self.mvals0.copy(), np.random.randn(PopSize, self.FF.np) * self.trust0)
            ) / (self.FF.np**0.5)
        ...
        def generate_new_population(sorted, pop):
            ...
            for i in range(0, KeepNum, 2):
                ...
                newpop[-i - 1], newpop[-i - 2] = cross_over(
                    newpop[a[i]], newpop[a[i + 1]]
                )
    ...
    def Precondition(self):
        ...
        def newcond(logrskeys, multiply=True):
            ...
            """
            new_rsord = OrderedDict(
                [(k, np.exp(logrskeys[i])) for i, k in enumerate(self.FF.rs_ord.keys())]
            )
            ...
            if newcond.verbose and newcond.step_n % 1000 == 0:
                logger.info(
                    ...
                    % (
                        newcond.step_n,
                        nlog,
                        list(new_rsord.keys())[np.argmax(np.abs(dlog))],
                        Cond_a,
                        Reg,
                        np.log(Cond_a) + Reg,
                    )
        ...
        optmethod = "basin"
        if optmethod == "basin":
            optimized = optimize.basinhopping(
                newcond,
                logrsmult,
                stepsize=1.0,
                niter=self.maxstep,  # disp=True,
                minimizer_kwargs={
                    "method": "Powell",
                    "tol": 0.1,
            ...
            logger.info(
                "New multipliers:"
                + " ".join(["% .3f" % np.exp(s) for s in bestsol])
                + "\n"
            )
            logger.info(
            ...
            while True:
                ...
                if np.abs(outerval - bestval) < 0.1:
                    logger.info(
                        "Sequential optimization value improved by less than 0.1 (%.3f)"
                        % (outerval - bestval)
                    )
                    break
                outerval = bestval
            ...
            raise RuntimeError
        new_rsord = OrderedDict(
            [(k, np.exp(optresult[i])) for i, k in enumerate(self.FF.rs_ord.keys())]
        )
        ...
        opt_rsord = OrderedDict(
            [
                (k, est124(np.exp(optresult[i]) * self.FF.rs_ord[k]))
                for i, k in enumerate(self.FF.rs_ord.keys())
            ]
        )
        # Print the final answer
        answer = self.FF.make_rescale(
            opt_rsord, mvals=self.mvals0, H=H.copy(), multiply=False
        )
        logger.info(
        ...
        for i in range(self.FF.np):
            ...
            cD = (
                ...
                or (
                    abs(D) > 1e-2
                    and "\x1b[91m"
                    or (abs(D) > 1e-5 and "\x1b[93m" or "\x1b[92m")
                )
            )
            cQ = (
                abs(Q) > 0.5
                ...
                or (
                    abs(Q) > 1e-2
                    and "\x1b[91m"
                    or (abs(Q) > 1e-5 and "\x1b[93m" or "\x1b[92m")
                )
            )
            logger.info(
    ...
    def FDCheckH(self):
        ...
        def wrap2(mvals0, pidxi, pidxj):
            def func1(arg):
                ...
                mvals[pidxj] += arg
                return f1d5p(
                    fdwrap(self.Objective.Full, mvals, pidxi, "X", Order=0), self.h
                )
    ...
    def writechk(self):
        ...
        if self.wchk_fnm is not None:
            ...
            with wopen(os.path.join(self.root, self.wchk_fnm), binary=True) as f:
                pickle.dump(self.chk, f)
