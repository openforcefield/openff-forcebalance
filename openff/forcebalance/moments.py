...
from openff.forcebalance.nifty import printcool_dictionary
...
class Moments(Target):
    ...
    def read_reference_data(self):
        ...
        for line in open(self.mfnm):
            ...
            if len(s) == 0:
                ...
                self.ref_moments["dipole"] = OrderedDict(
                    zip(["x", "y", "z"], [float(i) for i in s])
                )
        ...
        if "quadrupole" in self.ref_moments:
            trace3 = (
                self.ref_moments["quadrupole"]["xx"]
                + self.ref_moments["quadrupole"]["yy"]
                + self.ref_moments["quadrupole"]["zz"]
            ) / 3
    ...
    def indicate(self):
        ...
        for Ord in self.ref_moments:
            for Comp in self.ref_moments[Ord]:
                if (
                    abs(self.calc_moments[Ord][Comp]) > 1e-6
                    or abs(self.ref_moments[Ord][Comp]) > 1e-6
                ):
                    PrintDict[
                        f"{Ord}-{Comp}"
                    ] = "{: 9.3f} {: 9.3f} {: 9.3f} {: 12.5f}".format(
                        self.calc_moments[Ord][Comp],
        ...
        printcool_dictionary(
            PrintDict,
            title="Moments and/or Polarizabilities, Objective = % .5e\n %-20s %9s %9s %9s %11s"
        ...
        answer = np.array(
            list(
                itertools.chain(
                    *[
                        [
                            dct[i] / self.denoms[ord]
                            if self.denoms[ord] != 0.0
                            else 0.0
                            for i in dct
                        ]
                        for ord, dct in moment_dict.items()
                    ]
                )
            )
        )
    ...
    def get(self, mvals, AGrad=False, AHess=False):
        ...
        def get_momvals(mvals_):
            self.FF.make(mvals_)
            moments = self.engine.multipole_moments(
                polarizability="polarizability" in self.ref_moments,
        ...
        self.FF.make(mvals)
        ref_momvals = self.unpack_moments(self.ref_moments)
        calc_moments = self.engine.multipole_moments(
            polarizability="polarizability" in self.ref_moments,
            optimize=self.optimize_geometry,
        )
