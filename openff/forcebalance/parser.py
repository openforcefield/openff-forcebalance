...
gen_opts_types = {
    ...
    "bools": {
        "backup": (1, 10, "Write temp directories to backup before wiping them"),
        "writechk_step": (
            1,
            -50,
            "Write the checkpoint file at every optimization step",
        ),
        "retain_micro_outputs": (
            1,
            10,
...
tgt_opts_types = {
    "strings": {
        ...
        "recharge_esp_store": (
            "esp-store.sqlite",
            0,
            "A SQLite database containing the electrostatic property data to train against.",
            "OpenFF Recharge target",
            "Recharge_SMIRNOFF",
        ),
        "recharge_property": (
            "esp",
            0,
            "The type of electrostatic property to train against [esp, electric-field].",
            "OpenFF Recharge target",
            "Recharge_SMIRNOFF",
        ),
        "energy_mode": (
            "average",
            50,
            'How to treat relative (MM-QM) energies. "average": Subtract out the mean gap (default). "qm_minimum": Reference all MM and QM energies to the structure with minimum QM energy.'
    ...
    "allcaps": {
        ...
        "engine": (
            None,
            180,
            "The external code used to execute the simulations (GMX, AMBER, OpenMM)",
            "All targets (important)",
            "",
        ),
    },
    ...
    "bools": {
        ...
        "attenuate": (
            0,
            110,
            "Normalize interaction energies using 1/sqrt(denom**2 + (E(qm)-denom)**2) for energies more positive than denom.",
            "Multiple targets that involve realtive energies",
            "abinitio, binding, interaction, torsionprofile",
        ),
        "normalize": (
            0,
            -150,
        ...
        "w_normalize": (
            0,
            0,
            "Normalize the condensed phase property contributions to the liquid property target",
            "Condensed phase property targets",
            "liquid",
        ),
        "manual": (
    ...
    "floats": {
        ...
        "energy_denom": (
            1.0,
            100,
            "Energy denominator in kcal/mol for objective function and lower energy limit for attenuating weights where applicable",
            "Multiple targets that involve realtive energies",
            "abinitio, binding, hydration, interaction, torsionprofile",
        ),
        "energy_upper": (
            30.0,
            50,
            "Upper energy cutoff in kcal/mol for setting weights to zero, used to exclude super-repulsive interactions",
            "Multiple targets that involve relative energies",
            "abinitio, interaction, torsionprofile",
        ),
        "w_rho": (
            1.0,
            0,
        ...
        "energy_asymmetry": (
            1.0,
            -150,
            'Snapshots with (E_MM - E_QM) < 0.0 will have their weights increased by this factor. Only valid if energy_mode is set to "qm_minimum".',
            "Ab initio targets",
        ),
        "nonbonded_cutoff": (
            None,
...
all_opts_names = list(
    itertools.chain(*[i.keys() for i in gen_opts_types.values()])
) + list(itertools.chain(*[i.keys() for i in tgt_opts_types.values()]))
...
for i in all_opts_names:
    ...
    if len(iocc) != 1:
        logger.error(
            "CODING ERROR: ForceBalance option {} occurs in more than one place ({})\n".format(
                i, str(iocc)
            )
        )
        raise RuntimeError
...
def read_mvals(fobj):
    ...
    for line in fobj:
        if re.match(r"(/read_mvals)|(^\$end)", line):
            break
        Answer.append(
            float(line.split("[", maxsplit=1)[-1].split("]", maxsplit=1)[0].split()[-1])
        )
    return Answer
...
def printsection(heading, optdict, typedict):
    ...
    for i in ["strings", "allcaps", "lists", "ints", "bools", "floats", "sections"]:
        ...
        for j in typedict[i]:
            ...
            Option.append(f"# ({vartype}) {typedict[i][j][2]}")
...
def parse_inputs(input_file=None):
    r"""Parse through the input file and read all user-supplied options.
    ...
    options = deepcopy(
        gen_opts_defaults
    )  # deepcopy to make sure options doesn't make changes to gen_opts_defaults
    ...
    for line in fobj:
        try:
            ...
            if re.match(r"^\$", line):
                ...
                (this_opt, opts_types) = (
                    (options, gen_opts_types)
                    if section == "OPTIONS"
                    else (this_tgt_opt, tgt_opts_types)
                )
                ...
                if len(s) > 1 and s[1].upper() == "NONE":
                    ...
                    if len(s) == 1:
                        ...
                        logger.error(
                            '{} is a true/false option but you provided {}; to enable, provide ["1", "yes", "true", "on" or <no value>].  To disable, provide ["0", "no", "false", or "off"].\n'.format(
                                key, s[1]
                            )
                        )
                        raise RuntimeError
