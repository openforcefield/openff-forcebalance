...
class Hydration(Target):
        ...
        self.molecules = OrderedDict(
            [
                (
                    ...
                    os.path.abspath(
                        os.path.join(
                            self.root, self.tgtdir, "molecules", i + self.crdsfx
                        )
                    ),
                )
                for i in self.IDs
    ...
    def run_simulation(self, label, liq, AGrad=True):
        ...
        if not os.path.exists("md_result.p"):
            ...
            LinkFile(
                self.molecules[label], "./%s" % os.path.basename(self.molecules[label])
            )
            ...
            if wq is None:
                ...
                queue_up(
                    wq,
                    command=cmdstr + " &> md.out",
                    tag="{}:{}/{}".format(self.name, label, "liq" if liq else "gas"),
                    input_files=self.scripts
                    + [
                        "simulation.p",
                        "forcefield.p",
                        os.path.basename(self.molecules[label]),
                    ],
                    output_files=["md_result.p", "md.out"] + self.extra_output,
                    tgt=self,
                    verbose=False,
                    print_time=3600,
                )
    ...
    def submit_liq_gas(self, mvals, AGrad=True):
        ...
        printcool(
            ...
            % (
                self.name,
                2 * len(self.IDs),
                self.liquid_eq_steps,
                self.liquid_md_steps,
                self.gas_eq_steps,
                self.gas_md_steps,
            ),
            color=0,
    ...
    def build_engines(self):
        ...
        for mnm in self.IDs:
            pdbfnm = os.path.abspath(
                os.path.join(self.root, self.tgtdir, "molecules", mnm + ".pdb")
            )
            self.liq_engines[mnm] = self.engine_(
                target=self, coords=pdbfnm, implicit_solvent=True, **self.engine_opts
            )
            self.gas_engines[mnm] = self.engine_(
                target=self, coords=pdbfnm, implicit_solvent=False, **self.engine_opts
            )
    ...
    def indicate(self):
        ...
        if self.have_experr:
            ...
            for ID in self.IDs:
                data[ID].append(f"{self.expval[ID]: 9.3f} +- {self.experr[ID]:6.3f}")
        else:
        ...
        if hasattr(self, "calc_err"):
            ...
            for ID in self.IDs:
                data[ID].append(f"{self.calc[ID]: 10.3f} +- {self.calc_err[ID]:6.3f}")
        else:
        ...
        for iid, ID in enumerate(self.IDs):
            ...
            data[ID].append(
                "%8.3f" % (self.whfe[iid] * (self.calc[ID] - self.expval[ID]) ** 2)
            )
    ...
    def get_exp(self, mvals, AGrad=False, AHess=False):
        ...
        beta = 1.0 / (kb * self.hfe_temperature)
        for ilabel, label in enumerate(self.IDs):
            ...
            for p in ["gas", "liq"]:
                ...
                if p == "gas":
                    results["Potentials"]
                    results["Potentials"] + results["Hydration"]
                    ...
                    data[p]["HydErr"] = np.std(
                        [
                            -kT * np.log(np.mean(expmbH[np.random.randint(L, size=L)]))
                            for i in range(100)
                        ]
                    ...
                    if AGrad:
                        ...
                        dEaq = (
                            results["Potential_Derivatives"]
                            + results["Hydration_Derivatives"]
                        )
                        data[p]["dHyd"] = (
                            flat(np.dot(dEaq, col(expmbH)) / L)
                            - np.mean(dEg, axis=1) * np.mean(expmbH)
                        ) / np.mean(expmbH)
                elif p == "liq":
                    results["Potentials"] - results["Hydration"]
                    results["Potentials"]
                    ...
                    data[p]["HydErr"] = np.std(
                        [
                            +kT * np.log(np.mean(exppbH[np.random.randint(L, size=L)]))
                            for i in range(100)
                        ]
                    ) * np.sqrt(statisticalInefficiency(results["Hydration"]))
                    if AGrad:
                        ...
                        data[p]["dHyd"] = -(
                            flat(np.dot(dEg, col(exppbH)) / L)
                            - np.mean(dEaq, axis=1) * np.mean(exppbH)
                        ) / np.mean(exppbH)
                os.chdir("..")
            ...
            if self.hfemode == "exp_gas":
                ...
                self.hfe_dict[label] = (
                    0.5 * (data["liq"]["Hyd"] + data["gas"]["Hyd"]) / 4.184
                )
                self.hfe_err[label] = (
                    0.5 * (data["liq"]["HydErr"] + data["gas"]["HydErr"]) / 4.184
                )
            if AGrad:
        ...
        for ilabel, label in enumerate(self.IDs):
            ...
            for p in ["gas", "liq"]:
                ...
                if AGrad:
                    ...
                    data[p]["dHyd"] = np.mean(dH, axis=1) - beta * (
                        flat(np.dot(dE, col(H)) / len(H))
                        - np.mean(dE, axis=1) * np.mean(H)
                    )
                os.chdir("..")
    ...
    def get(self, mvals, AGrad=False, AHess=False):
        ...
        Answer["X"] = (
            np.dot(D, D)
            / self.denom**2
            / (np.sum(self.whfe) if self.normalize else 1)
        )
        for p in self.pgrad:
            Answer["G"][p] = (
                2
