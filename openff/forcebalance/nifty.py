...
def printcool(
    ...
    for ln, line in enumerate(text):
        ...
        if ansi is not None:
            ansi = str(ansi)
            logger.info(
                f"{sym}| \x1b[{ansi}m{padleft} " + line + f" {padright}\x1b[0m |{sym}\n"
            )
        elif color is not None:
            if color == 0 and bold:
                logger.info(
                    f"{sym}| \x1b[1m{padleft} " + line + f" {padright}\x1b[0m |{sym}\n"
                )
            elif color == 0:
                logger.info(f"{sym}| {padleft} " + line + f" {padright} |{sym}\n")
            else:
                logger.info(
                    "%s| \x1b[%s9%im%s " % (sym, bold and "1;" or "", color, padleft)
                    + line
                    + f" {padright}\x1b[0m |{sym}\n"
                )
...
def printcool_dictionary(
    ...
    def magic_string(str):
        ...
        return eval(
            "'%%-%is' %% '%s'" % (keywidth, str.replace("'", "\\'").replace('"', '\\"'))
        )
    ...
    if isinstance(Dict, OrderedDict):
        logger.info(
            "\n".join(
                [
                    " " * leftpad + f"{magic_string(str(key))} {str(Dict[key])} "
                    for key in Dict
    ...
    if os.path.exists(path):
        ...
        while True:
            ...
            if not os.path.exists(newf):
                break
            i += 1
        logger.info(f"Backing up {oldf} -> {newf}\n")
...
def onefile(fnm=None, ext=None, err=False):
    ...
    if fnm is not None:
        if os.path.exists(fnm):
            if os.path.dirname(os.path.abspath(fnm)) != os.getcwd():
                ...
                if os.path.exists(fdest):
                    if not filecmp.cmp(fsrc, fdest):
                        logger.error(
                            "onefile() will not overwrite {} with {}\n".format(
                        ...
                        logger.info(
                            "\x1b[93monefile() says the files {} and {} are identical\x1b[0m\n".format(
                                os.path.abspath(fnm), os.getcwd()
                            )
                        )
                else:
                    logger.info(
                        "\x1b[93monefile() will copy {} to {}\x1b[0m\n".format(
                            os.path.abspath(fnm), os.getcwd()
                        )
                    )
            ...
            warn_once(
                "File specified by {} does not exist - will try to autodetect .{} extension".format(
                    fnm, ext
                )
            )
    answer = None
    ...
    if len(ls) != 1:
        ...
        warn_once(
            f"Autodetected {answer} in {cwd}",
            warnhash="Autodetected %s" % answer,
        )
    return answer
...
def listfiles(fnms=None, ext=None, err=False, dnm=None):
    ...
    if answer == [] and err:
        logger.error(
            "listfiles function failed to come up with a file! (fnms = {} ext = {})".format(
                str(fnms), str(ext)
            )
        )
        raise RuntimeError
...
def Leave(Dir):
    if os.path.split(os.getcwd())[1] != Dir:
        logger.error(
            "Trying to leave directory {}, but we're actually in directory {} (check your code)\n".format(
    ...
    if os.path.exists(src):
        ...
        if os.path.islink(dest) and not os.path.exists(dest):
            ...
            if os.path.islink(dest):
                ...
                logger.error(
                    "Tried to create symbolic link {} to {}, destination exists but isn't a symbolic link\n".format(
                        src, dest
                    )
                )
                raise RuntimeError
        ...
        if not nosrcok:
            logger.error(
                "Tried to create symbolic link {} to {}, but source file doesn't exist{}\n".format(
...
def CopyFile(src, dest):
    if os.path.exists(src):
        if os.path.exists(dest):
            if os.path.islink(dest):
                logger.error(
                    "Tried to copy {} to {}, destination exists but it's a symbolic link\n".format(
                        src, dest
                    )
                )
                raise RuntimeError
        ...
        logger.error(
            "Tried to copy {} to {}, but source file doesn't exist{}\n".format(
                src, dest, MissingFileInspection(src)
            )
        )
        raise RuntimeError
