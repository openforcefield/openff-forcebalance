...
from openff.forcebalance.vibration import read_reference_vdata, vib_overlap
...
def draw_normal_modes(
    ...
    fig, axs = plt.subplots(
        len(normal_modes_rearr),
        1,
        figsize=(4, 4 * len(normal_modes_rearr)),
        subplot_kw={"projection": "3d"},
    )

    def render_normal_modes(
        elem, xyz, eigvecs, color, qm=False, ref_eigvals=None, eigvals_rearr=None
    ):
        for idx, eigvec in enumerate(eigvecs):
            ...
            if qm:
                axs[idx].set_title(
                    f"normal mode #{idx} (blue:QM({ref_eigvals[idx]:.2f}), red:MM({eigvals_rearr[idx]:.2f}))"
                )
    ...
    render_normal_modes(
        ...
        "blue",
        qm=True,
        ref_eigvals=ref_eigvals,
        eigvals_rearr=freqs_rearr,
    )
...
def draw_vibfreq_scatter_plot_n_overlap_matrix(
    ...
    overlap_matrix = np.array(
        [
            [(vib_overlap(engine, v1, v2)) for v2 in normal_modes_rearr]
            for v1 in ref_eigvecs
        ]
    )
    qm_overlap_matrix = np.array(
        [[(vib_overlap(engine, v1, v2)) for v2 in ref_eigvecs] for v1 in ref_eigvecs]
    )
    ...
    width = axes_size.AxesY(axs[1], aspect=1.0 / aspect)
    ...
    err = np.linalg.norm(qm_overlap_matrix - overlap_matrix) / np.linalg.norm(
        qm_overlap_matrix
    )  # measure of error in matrix (Relative error)
    axs[1].set_title(
        f"QM vs. MM normal modes\n Correlation coef. ={corr_coef:.4f}, Error={err:.4f}"
    )
