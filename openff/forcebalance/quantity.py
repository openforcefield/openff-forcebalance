...
from openff.forcebalance.nifty import col, flat, printcool, statisticalInefficiency
...
class Quantity_Density(Quantity):
    def __init__(self, engname, temperature, pressure, name=None):
        """Density."""
        super().__init__(engname, temperature, pressure, name)
    ...
    def extract(self, engines, FF, mvals, h, pgrad, AGrad=True):
        ...
        if self.engname == "gromacs":
            ...
            energyterms = engines[0].energy_termnames(
                edrfile="{}.{}".format(deffnm, "edr")
            )
            ...
            ekeep_order = [
                key
                for (key, value) in sorted(energyterms.items(), key=lambda k_v: k_v[1])
                if key in ekeep
            ]
        ...
        printcool("Initializing array to length %i" % len(Energy), color=4, bold=True)
        ...
        Rho_grad = mBeta * (
            flat(np.dot(G, col(Density))) / len(Density)
...
class Quantity_H_vap(Quantity):
    ...
    def extract(self, engines, FF, mvals, h, pgrad, AGrad=True):
        ...
        if self.engname == "gromacs":
            ...
            energyterms1 = engines[0].energy_termnames(
                edrfile="{}.{}".format(deffnm1, "edr")
            )
            energyterms2 = engines[1].energy_termnames(
                edrfile="{}.{}".format(deffnm2, "edr")
            )
            # Grab energy terms to print and keep track of energy term order.
            ekeep1 = [
                "Total-Energy",
            ...
            ekeep_order1 = [
                ...
                for (key, value) in sorted(
                    energyterms1.items(), key=lambda k_v1: k_v1[1]
                )
                if key in ekeep1
            ]
            ekeep_order2 = [
                key
                for (key, value) in sorted(
                    energyterms2.items(), key=lambda k_v2: k_v2[1]
                )
                if key in ekeep2
            ]
        ...
        printcool("Initializing arrays to lengths %d" % len(Energy), color=4, bold=True)
        ...
        Hvap_grad += mBeta * (
            flat(np.dot(Gm, col(mEnergy))) / len(mEnergy)
        ...
        Hvap_grad += (
            Beta
            * (
                flat(np.dot(G, col(Energy))) / len(Energy)
                - np.mean(Energy) * np.mean(G, axis=1)
            )
            / nmol
        )
