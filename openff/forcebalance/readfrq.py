...
def read_frq_gau(gauout):
    ...
    for line in open(gauout).readlines():
        ...
        if VMode == 3:
            ...
            if len(s) != nfrq * 3 + 2:
                ...
                for i in range(nfrq):
                    readmodes[i].append(
                        [float(s[j]) for j in range(2 + 3 * i, 5 + 3 * i)]
                    )
    ...
    return (
        np.array(frqs),
        [i / np.linalg.norm(i) for i in unnorm],
        np.array(intens),
        elem,
        xyz,
    )
...
def read_frq_tc(tcout, scrdir="scr"):
    ...
    lineCounter = -100
    xyzpath = os.path.join(
        os.path.split(os.path.abspath(tcout))[0], scrdir, "CentralGeometry.initcond.xyz"
    )
    tcdat = os.path.join(
        os.path.split(os.path.abspath(tcout))[0], scrdir, "Frequencies.dat"
    )
    if not os.path.exists(xyzpath):
        raise RuntimeError(
            "%s doesn't exist; please provide a scratch folder to this function"
            % xyzpath
        )
    ...
    for line in open(tcout):
        if (
            "Vibrational Frequencies/Thermochemical Analysis After Removing Rotation and Translation"
            in line
        ):
            found_vib = True
        if found_vib:
