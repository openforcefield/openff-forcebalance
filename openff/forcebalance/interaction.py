...
class Interaction(Target):
    ...
    def read_reference_data(self):
        ...
        for line in open(os.path.join(self.root, self.qfnm)):
            ...
            if (
                all(len(i) in [self.ns, 0] for i in [self.eqm])
                and len(self.eqm) == self.ns
            ):
                break
    ...
    def indicate(self):
        ...
        if len(self.label) == self.ns:
            ...
            for i, label in enumerate(self.label):
                PrintDict[
                    label
                ] = "{: 9.3f} {: 9.3f} {: 9.3f} {: 9.3f} {: 11.5f}".format(
                    self.emm[i], self.eqm[i], delta[i], self.divisor[i], deltanrm[i]
                )
            printcool_dictionary(
                PrintDict,
    ...
    def get(self, mvals, AGrad=False, AHess=False):
        ...
        if self.writelevel > 0:
            ...
            pickle.dump(
                (self.name, self.label, self.prefactor, self.eqm, emm),
                open("qm_vs_mm.p", "w"),
            )
            ...
            plot_interaction_qm_vs_mm(
                qm_data, mm_data, title="Interaction Energy " + self.name
            )
        ...
        for p in self.pgrad:
            Answer["G"][p] = 2 * np.dot(
                self.prefactor * D / self.divisor, dV[p, :] / self.divisor
            )
            for q in self.pgrad:
                Answer["H"][p, q] = 2 * np.dot(
                    self.prefactor * dV[p, :] / self.divisor, dV[q, :] / self.divisor
                )
        ...
        try:
            if self.engine.name == "openmm":
                ...
                if hasattr(self.engine, "A"):
                    del self.engine.A
                if hasattr(self.engine, "B"):
                    del self.engine.B
        except:
            pass
