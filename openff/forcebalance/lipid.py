...
class Lipid(Target):
    ...
    def __init__(self, options, tgt_opts, forcefield):
        ...
        if "n_ic" in self.RefData:
            ...
            for pt in self.PhasePoints:
                pt_label = f"IC/{pt[0]}K-{pt[1]}{pt[2]}"
                if not os.path.exists(
                    os.path.join(self.root, self.tgtdir, pt_label, self.lipid_coords)
                ):
                    raise RuntimeError(
            ...
            # Read in lipid starting coordinates.
            if not os.path.exists(
                os.path.join(self.root, self.tgtdir, self.lipid_coords)
            ):
                logger.error(
                ...
                raise RuntimeError
            self.lipid_mol = Molecule(
                os.path.join(self.root, self.tgtdir, self.lipid_coords), toppbc=True
            )
                ...
                try:
                    ...
                    for head, val in zip(headings, line):
                        ...
                        if isfloat(val):
                            ...
                            self.RefData.setdefault(head, OrderedDict([]))[
                                (t, pval, punit)
                            ] = np.array(list(map(float, val.split())))
                except:
                    logger.error(line + "\n")
        ...
        for head in self.RefData:
            ...
            if head in known_vars:
                ...
                if len(wts) > 1:
                    ...
                    if head == "scd":
                        default_denoms[head + "_denom"] = np.average(
                            np.sqrt(np.dot(wts, (dat - avg) ** 2) / wts.sum())
                        )
                    else:
    ...
    def npt_simulation(self, temperature, pressure, simnum):
        ...
        if not os.path.exists("npt_result.p"):
            ...
            cmdstr = "{} python npt_lipid.py {} {:.3f} {:.3f}".format(
                ...
                [
                    (
                        ...
                        % (
                            " ".join(
                                "\t \t \t %9.6f    %9.6f +- %-7.6f % 7.6f \n" % F
                                for F in zip(exp[PT], calc[PT], flat(err[PT]), delt[PT])
                            )
                        ),
                    )
                    for PT in calc
    ...
    def get(self, mvals, AGrad=True, AHess=True):
        ...
        for label, PT in zip(self.Labels, self.PhasePoints):
            if "n_ic" in self.RefData:
                self.lipid_mols[PT] = [
                    Molecule(last_frame) for last_frame in self.lipid_mols[PT]
                ]
                n_uniq_ic = int(self.RefData["n_ic"][PT])
                for ic in range(n_uniq_ic):
                    if os.path.exists(f"./{label}/trj_{ic}/npt_result.p"):
                        # Read in each each parallel simulation's data, and concatenate each property time series.
                        ts = lp_load(f"./{label}/trj_{ic}/npt_result.p")
                        if ic == 0:
                            ts_concat = list(ts)
                        else:
                            for d_arr in range(len(ts)):
                                if isinstance(ts[d_arr], np.ndarray):
                                    ...
                                    if d_arr == 5:
                                        ts_concat[d_arr] = np.append(
                                            ts_concat[d_arr], ts[d_arr], axis=1
                                        )
                                    else:
                                        ts_concat[d_arr] = np.append(
                                            ts_concat[d_arr], ts[d_arr], axis=0
                                        )
                                if isinstance(ts_concat[d_arr], list):
                                    ts_concat[d_arr] = [
                                        np.append(
                                            ts_concat[d_arr][i], ts[d_arr][i], axis=1
                                        )
                                        for i in range(len(ts_concat[d_arr]))
                                    ]
        ...
        (
            ...
            Scd_errs,
            LKappa_errs,
        ) = ([Results[t][i] for t in range(len(Points))] for i in range(18))
        ...
        for i, PT in enumerate(Points):
            ...
            A_m2 = A * 1e-18
            kbT = 1.3806488e-23 * T
            LKappa_calc[PT] = (1e3 * 2 * kbT / 128) * (
                avg(A_m2) / (avg(A_m2**2) - avg(A_m2) ** 2)
            )
            al_avg = avg(A_m2)
            ...
            GLKappa2 = (al_avg / al_var**2) * (
                covde(A_m2**2) - (2 * al_avg * covde(A))
            )
            ...
            Scd_grad[PT] = mBeta * (
                flat(np.average(np.dot(G, (S * W[:, np.newaxis])), axis=1))
                - np.average(np.average(S * W[:, np.newaxis], axis=0), axis=0) * Gbar
            )
        ...
        X_Scd, G_Scd, H_Scd, ScdPrint = self.objective_term(
            Points, "scd", Scd_calc, Scd_std, Scd_grad, name="Deuterium Order Parameter"
        )
        X_LKappa, G_LKappa, H_LKappa, LKappaPrint = self.objective_term(
            Points,
            "lkappa",
            LKappa_calc,
            LKappa_std,
            LKappa_grad,
            name="Bilayer Compressibility",
        )
