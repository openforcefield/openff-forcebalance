...
class OptGeoTarget(Target):
    ...
    def __init__(self, options, tgt_opts, forcefield):
        ...
        self.set_option(
            ...
            None,
            "optgeo_options",
            os.path.join(self.tgtdir, tgt_opts["optgeo_options_txt"]),
        )
        ...
        for sys_name, sys_opt_dict in sys_opts.items():
            ...
            for k in ["name", "geometry", "topology"]:
                if k not in sys_opt_dict:
                    warn_press_key(
                        f"key {k} missing in system section named {sys_name}"
                    )
        return sys_opts
    ...
    def _build_internal_coordinates(self):
        ...
        for sysname, sysopt in self.sys_opts.items():
            ...
            vref_angles = np.array(
                [ic.value(pos_ref) * RADIAN_2_DEGREE for ic in ic_angles]
            )
            vref_dihedrals = np.array(
                [ic.value(pos_ref) * RADIAN_2_DEGREE for ic in ic_dihedrals]
            )
            vref_impropers = np.array(
                [ic.value(pos_ref) * RADIAN_2_DEGREE for ic in ic_impropers]
            )
    ...
    def system_driver(self, sysname, save_mol=None):
        ...
        v_ic = {
            ...
            "angles": np.array(
                [ic.value(pos) * RADIAN_2_DEGREE for ic in ic_dict["ic_angles"]]
            ),
            "dihedrals": np.array(
                [ic.value(pos) * RADIAN_2_DEGREE for ic in ic_dict["ic_dihedrals"]]
            ),
            "impropers": np.array(
                [ic.value(pos) * RADIAN_2_DEGREE for ic in ic_dict["ic_impropers"]]
            ),
        }
        return v_ic

    def indicate(self):
        title_str = f"{self.name}, Objective = {self.objective: .5e}"
    ...
    def get(self, mvals, AGrad=False, AHess=False):
        ...
        def compute(mvals, p_idx=None):
            ...
            for sysname, sysopt in self.sys_opts.items():
                ...
                if (
                    enable_system_mval_mask
                    and in_fd()
                    and (p_idx is not None)
                    and (self.system_mval_masks[sysname][p_idx] == False)
                ):
                ...
                diff_bond = (
                    ((vref_bonds - vtar_bonds) * scale_bond).tolist()
                    if n_bonds > 0
                    else []
                )
                ...
                diff_angle = (
                    (
                        periodic_diff(vref_angles, vtar_angles, 360) * scale_angle
                    ).tolist()
                    if n_angles > 0
                    else []
                )
                ...
                if not in_fd():
                    ...
                    rmsd_dihedral = compute_rmsd(
                        vref_dihedrals, vtar_dihedrals, v_periodic=360
                    )
                    rmsd_improper = compute_rmsd(
                        vref_impropers, vtar_impropers, v_periodic=360
                    )
                    obj_total = sum(v**2 for v in sys_obj_list)
                    self.PrintDict[
                        sysname
                    ] = "{: 9.3f} {: 7.2f} {: 9.3f} {: 7.2f} {: 9.3f} {: 7.2f} {: 9.3f} {: 7.2f} {:17.3f}".format(
                        rmsd_bond,
                        bond_denom,
                        rmsd_angle,
                        angle_denom,
        ...
        if self.writelevel > 0:
            ...
            with open("rmsd_decomposition.txt", "w") as fout:
                for sysname in self.internal_coordinates:
                    ...
                    v_ic = self.system_driver(
                        sysname, save_mol="%s_mmopt.xyz" % sysname
                    )
