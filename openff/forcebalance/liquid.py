...
def weight_info(W, PT, N_k, verbose=True, PTS=None):
    ...
    if verbose:
        ...
        if tfl > 0 and pl:
            ...
            logger.info(line2 + "\n")
        logger.info("\n")
        logger.info(f"InfoContent: {I: .2f} snapshots ({100 * I / len(W):.2f} %)\n")
    return C
...
class Liquid(Target):
    ...
    def __init__(self, options, tgt_opts, forcefield):
        ...
        if not os.path.exists(os.path.join(self.root, self.tgtdir, self.liquid_coords)):
            ...
            raise RuntimeError
        self.liquid_mol = Molecule(
            os.path.join(self.root, self.tgtdir, self.liquid_coords), toppbc=True
        )
        ...
        if self.n_molecules >= 0:
            ...
            self.n_molecules = len(self.liquid_mol.molecules)
            if len({len(m) for m in self.liquid_mol.molecules}) != 1:
                warn_press_key(
                    "Possible issue because molecules are not all the same size! Sizes detected: %s"
                    % str({len(m) for m in self.liquid_mol.molecules}),
                    timeout=30,
                )
            else:
                logger.info(
        ...
        if "surf_ten" in self.RefData:
            ...
            if not os.path.exists(
                os.path.join(self.root, self.tgtdir, self.nvt_coords)
            ):
                logger.error(
                    "Surface tension calculation requires %s, but it is not found."
                    % self.nvt_coords
                )
                raise RuntimeError
            self.surf_ten_mol = Molecule(
                os.path.join(self.root, self.tgtdir, self.nvt_coords), toppbc=True
            )
            # Extra files to be linked into the temp-directory.
    ...
    def post_init(self, options):
        ...
        if self.do_self_pol:
            ...
            self.gas_engine = self.engine_(
                target=self, mol=self.gas_mol, name="selfpol", **self.gas_engine_args
            )
    ...
    def prepare_temp_directory(self):
        ...
        for f in self.nptfiles + (self.nvtfiles if hasattr(self, "nvtfiles") else []):
            LinkFile(
                os.path.join(self.root, self.tgtdir, f), os.path.join(abstempdir, f)
            )
        for f in self.scripts:
            LinkFile(
                os.path.join(os.path.split(__file__)[0], "data", f),
                os.path.join(abstempdir, f),
            )

    def read_data(self):
        # Read the 'data.csv' file. The file should contain guidelines.
        with open(os.path.join(self.tgtdir, "data.csv")) as f:
            R0 = list(csv.reader(f))
        # All comments are erased.
        R1 = [
            [sub("#.*$", "", word) for word in line]
            for line in R0
            if len(line[0]) > 0 and line[0][0] != "#"
        ]
        # All empty lines are deleted and words are converted to lowercase.
        R = [
            [wrd.lower() for wrd in line]
            for line in R1
            if any([len(wrd) for wrd in line]) > 0
        ]
        ...
        for line in R:
            if line[0] == "global":
                ...
                try:
                    ...
                    t = [
                        float(val) for head, val in zip(headings, line) if head == "t"
                    ][0]
                    ...
                    punit = [
                        val.split()[1] if len(val.split()) >= 1 else "atm"
                        for head, val in zip(headings, line)
                        if head == "p"
                    ][0]
                    unrec = {punit}.difference(["atm", "bar"])
                    if len(unrec) > 0:
                        logger.error(
                    ...
                    for head, val in zip(headings, line):
                        if head == "t" or head == "p":
                            continue
                        if isfloat(val):
                            self.RefData.setdefault(head, OrderedDict([]))[
                                (t, pval, punit)
                            ] = float(val)
                        elif val.lower() == "true":
                            self.RefData.setdefault(head, OrderedDict([]))[
        ...
        for head in self.RefData:
            ...
            if head in known_vars:
                if head + "_wt" not in self.RefData:
                    ...
                    RefData_copy[head + "_wt"] = OrderedDict(
                        [(key, 1.0) for key in self.RefData[head]]
                    )
                ...
                if len(wts) > 1:
                    ...
                    default_denoms[head + "_denom"] = np.sqrt(
                        np.dot(wts, (dat - avg) ** 2) / wts.sum()
                    )
                else:
    ...
    def npt_simulation(self, temperature, pressure, simnum):
        ...
        if not os.path.exists("npt_result.p"):
            ...
            self.liquid_mol[simnum % len(self.liquid_mol)].write(
                self.liquid_coords, ftype="tinker" if self.engname == "tinker" else None
            )
            cmdstr = "{} python npt.py {} {:.3f} {:.3f}".format(
            ...
            if wq is None:
                ...
                queue_up(
                    wq,
                    command=cmdstr + " > npt.out 2>&1 ",
                    input_files=self.nptfiles
                    + self.scripts
                    + mol2_send
                    + ["forcebalance.p"],
    ...
    def nvt_simulation(self, temperature):
        ...
        if not os.path.exists("nvt_result.p"):
            link_dir_contents(os.path.join(self.root, self.rundir), os.getcwd())
            cmdstr = "{} python nvt.py {} {:.3f}".format(
                self.nptpfx, self.engname, temperature
            )
            if wq is None:
                logger.info("Running condensed phase simulation locally.\n")
                logger.info(
                ...
                queue_up(
                    wq,
                    command=cmdstr + " > nvt.out 2>&1 ",
                    input_files=self.nvtfiles
                    + self.scripts
                    + mol2_send
                    + ["forcebalance.p"],
                    output_files=["nvt_result.p", "nvt.out"] + self.extra_output,
                    tgt=self,
                )
    ...
    def objective_term(
        self, points, expname, calc, err, grad, name="Quantity", SubAverage=False
    ):
        ...
        logger.info(f"Physical quantity {name} uses denominator = {Denom: .4f}\n")
        if not LeastSquares:
        ...
        print_out = OrderedDict(
            [
                (
                    "    %8.2f %8.1f %3s" % PT,
                    "{:9.3f}    {:9.3f} +- {:<7.3f} {: 7.3f} {: 9.5f} {: 9.5f}".format(
                        exp[PT], calc[PT], err[PT], delt[PT], Weights[PT], Objs[PT]
                    ),
                )
                for PT in calc
            ]
    ...
    def read(self, mvals, AGrad=True, AHess=True):
        ...
        if len(mvals) > 0 and (np.max(np.abs(mvals1 - mvals)) > 1e-3):
            warn_press_key(
                "mvals from openff.forcebalance.p does not match up with internal values! (Are you reading data from a previous run?)\nmvals(call)={} mvals(disk)={}".format(
                    mvals, mvals1
                )
            )
        ...
        for dn in range(Counter() - 1, -1, -1):
            ...
            (
                ...
                Eps0_errs,
                NMols,
            ) = ([Results[t][i] for t in range(len(Points))] for i in range(17))
            ...
            if len(mPoints) > 0:
                self.AllResults[astrm]["mE"].append(
                    np.array([i for pt, i in zip(Points, mEnergies) if pt in mPoints])
                )
                self.AllResults[astrm]["mG"].append(
                    np.array([i for pt, i in zip(Points, mGrads) if pt in mPoints])
                )
    ...
    def get_normal(self, mvals, AGrad=True, AHess=True):
        ...
        mBPoints = (
            []
        )  # These are the phase points for which we are doing MBAR for the monomers.
        mPoints = (
            []
        )  # These are the phase points to use for enthalpy of vaporization; if we're scanning pressure then set hvap_wt for higher pressures to zero.
        ...
        for label, PT in zip(self.Labels, self.PhasePoints):
            if os.path.exists("./%s/npt_result.p" % label):
                ...
                if "hvap" in self.RefData and PT[0] not in [i[0] for i in mPoints]:
                    mPoints.append(PT)
                if (
                    "mbar" in self.RefData
                    and PT in self.RefData["mbar"]
                    and self.RefData["mbar"][PT]
                ):
                    BPoints.append(PT)
        ...
        E, V, R, Dx, Dy, Dz = (
            np.hstack(tuple(self.AllResults[astrm][i]))
            for i in ["E", "V", "R", "Dx", "Dy", "Dz"]
        )

        G, GDx, GDy, GDz = (
            np.hstack(np.concatenate(tuple(self.AllResults[astrm][i]), axis=2))
            for i in ["G", "GDx", "GDy", "GDz"]
        )
        ...
        if len(BPoints) > 1:
            ...
            mbar = pymbar.MBAR(
                U_kln, N_k, verbose=mbar_verbose, relative_tolerance=5.0e-8
            )
            W1 = mbar.getWeights()
            logger.info("Done\n")
        elif len(BPoints) == 1:
                ...
                if PT in mbar_points:
                    ...
                    for kk, PT1 in enumerate(mbar_points):
                        ...
                        logger.debug(
                            ...
                            % (
                                k * snapshots,
                                k * snapshots + snapshots,
                                m,
                                kk * snapshots,
                                kk * snapshots + snapshots,
                                mm,
                            )
                        )
                        new_weights[k * snapshots : (k + 1) * snapshots, m] = weights[
                            kk * snapshots : (kk + 1) * snapshots, mm
                        ]
                else:
                    logger.debug(
                    ...
                    new_weights[m * snapshots : (m + 1) * snapshots, m] = (
                        1.0 / snapshots
                    )
            return new_weights
        ...
        if self.do_self_pol:
            ...
            GEPol = np.array(
                [
                    (
                        f12d3p(
                            fdwrap(self.polarization_correction, mvals, p),
                            h=self.h,
                            f0=EPol,
                        )[0]
                        if p in self.pgrad
        ...
        for i, PT in enumerate(Points):
            ...
            W = flat(W2[:, i])
            C = weight_info(
                W, PT, np.ones(len(Points), dtype=int) * Shots, verbose=mbar_verbose
            )
            ...
            if PT in mPoints:
                ...
                Hvap_calc[PT] = (
                    np.dot(mW, mE) - np.dot(W, E) / NMol + kb * T - np.dot(W, PV) / NMol
                )
                Hvap_grad[PT] = mGbar + mBeta * (
                    flat(np.dot(mG, col(mW * mE))) - np.dot(mW, mE) * mGbar
                )
                Hvap_grad[PT] -= (
                    Gbar + mBeta * (flat(np.dot(G, col(W * E))) - np.dot(W, E) * Gbar)
                ) / NMol
                Hvap_grad[PT] -= (
                    mBeta * (flat(np.dot(G, col(W * PV))) - np.dot(W, PV) * Gbar)
                ) / NMol
                if self.do_self_pol:
                    Hvap_calc[PT] -= EPol
                    Hvap_grad[PT] -= GEPol
                if hasattr(self, "use_cni") and self.use_cni:
                    if not ("cni" in self.RefData and self.RefData["cni"][PT]):
                        ...
                        raise RuntimeError
                    logger.debug(
                        "Adding % .3f to enthalpy of vaporization at "
                        % self.RefData["cni"][PT]
                        + str(PT)
                        + "\n"
                    )
                ...
                if hasattr(self, "use_cvib_intra") and self.use_cvib_intra:
                    if not (
                        "cvib_intra" in self.RefData and self.RefData["cvib_intra"][PT]
                    ):
                        logger.error(
                ...
                if hasattr(self, "use_cvib_inter") and self.use_cvib_inter:
                    if not (
                        "cvib_inter" in self.RefData and self.RefData["cvib_inter"][PT]
                    ):
                        logger.error(
            ...
            if hasattr(self, "use_cvib_intra") and self.use_cvib_intra:
                logger.debug(
                    "Adding "
                    + str(self.RefData["devib_intra"][PT])
            ...
            if hasattr(self, "use_cvib_inter") and self.use_cvib_inter:
                logger.debug(
                    "Adding "
                    + str(self.RefData["devib_inter"][PT])
                    + " to the heat capacity\n"
                )
            ...
            GD2 = 2 * (
                flat(np.dot(GDx, col(W * Dx))) - avg(Dx) * flat(np.dot(GDx, col(W)))
            ) - Beta * (covde(Dx**2) - 2 * avg(Dx) * covde(Dx))
            GD2 += 2 * (
                flat(np.dot(GDy, col(W * Dy))) - avg(Dy) * flat(np.dot(GDy, col(W)))
            ) - Beta * (covde(Dy**2) - 2 * avg(Dy) * covde(Dy))
            GD2 += 2 * (
                flat(np.dot(GDz, col(W * Dz))) - avg(Dz) * flat(np.dot(GDz, col(W)))
            ) - Beta * (covde(Dz**2) - 2 * avg(Dz) * covde(Dz))
            Eps0_grad[PT] = (
                prefactor * (GD2 / avg(V) - mBeta * covde(V) * D2 / avg(V) ** 2) / T
            )
    ...
    def get_pure_num_grad(self, mvals, AGrad=True, AHess=True):
        ...
        for i_m in range(len(mvals)):
            ...
            for delta_m in [+self.liquid_fdiff_h, -self.liquid_fdiff_h]:
                ...
                # self.AllResults = defaultdict(lambda:defaultdict(list))
                property_results_pm[delta_m] = self.get_normal(
                    new_mvals, AGrad=False, AHess=False
                )
                os.chdir("..")
            for key in property_results:
                for PT in property_results[key][2].keys():
                    property_results[key][2][PT][i_m] = (
                        property_results_pm[+self.liquid_fdiff_h][key][0][PT]
                        - property_results_pm[-self.liquid_fdiff_h][key][0][PT]
                    ) / (2.0 * self.liquid_fdiff_h)
    ...
    def form_get_result(self, property_results, AGrad=True, AHess=True):
        ...
        X_Rho, G_Rho, H_Rho, RhoPrint = self.objective_term(
            Points, "rho", Rho_calc, Rho_std, Rho_grad, name="Density"
        )
        X_Hvap, G_Hvap, H_Hvap, HvapPrint = self.objective_term(
            Points,
            "hvap",
            Hvap_calc,
            Hvap_std,
            Hvap_grad,
            name="H_vap",
            SubAverage=self.hvap_subaverage,
        )
        X_Alpha, G_Alpha, H_Alpha, AlphaPrint = self.objective_term(
            Points, "alpha", Alpha_calc, Alpha_std, Alpha_grad, name="Thermal Expansion"
        )
        X_Kappa, G_Kappa, H_Kappa, KappaPrint = self.objective_term(
            Points, "kappa", Kappa_calc, Kappa_std, Kappa_grad, name="Compressibility"
        )
        X_Cp, G_Cp, H_Cp, CpPrint = self.objective_term(
            Points, "cp", Cp_calc, Cp_std, Cp_grad, name="Heat Capacity"
        )
        X_Eps0, G_Eps0, H_Eps0, Eps0Print = self.objective_term(
            Points, "eps0", Eps0_calc, Eps0_std, Eps0_grad, name="Dielectric Constant"
        )
        X_Surf_ten, G_Surf_ten, H_Surf_ten, Surf_tenPrint = self.objective_term(
            list(Surf_ten_calc.keys()),
            "surf_ten",
            Surf_ten_calc,
            Surf_ten_std,
            Surf_ten_grad,
