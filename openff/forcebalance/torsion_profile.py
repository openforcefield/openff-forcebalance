...
class TorsionProfileTarget(Target):
    ...
    def __init__(self, options, tgt_opts, forcefield):
        ...
        with open(os.path.join(self.tgtdir, "metadata.json")) as f:
            ...
            self.freeze_atoms = sorted(
                set(itertools.chain(*self.metadata["dihedrals"]))
            )
    ...
    def indicate(self):
        title_str = "Torsion Profile: {}, Objective = {: .5e}, Units = kcal/mol, Angstrom".format(
    ...
    def get(self, mvals, AGrad=False, AHess=False):
        ...
        self.PrintDict[self.name] = (
            ...
            % (
                ",".join(
                    ["%i" % i for i in self.metadata["torsion_grid_ids"][self.smin]]
                ),
                ",".join(
                    [
                        "%i" % i
                        for i in self.metadata["torsion_grid_ids"][
