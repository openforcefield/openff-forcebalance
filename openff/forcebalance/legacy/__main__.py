...
def getOptions():
    ...
    parser.add_argument(
        ...
        "--exclude",
        action="store_true",
        help="exclude specified modules from test loading",
    )
    parser.add_argument(
        "-p",
        "--pretend",
        action="store_true",
        help="load tests but don't actually run them",
    )
    parser.add_argument(
        "--no-color",
        action="store_true",
        help="print test results in black and white (no extra formatting)",
    )
    ...
    loglevel.add_argument(
        ...
        dest="loglevel",
        action="store_const",
        const=forcebalance.output.DEBUG,
        default=forcebalance.output.INFO,
    )
    loglevel.add_argument(
        "-q",
        "--quiet",
        dest="loglevel",
        action="store_const",
        const=forcebalance.output.WARNING,
        default=forcebalance.output.INFO,
    )
    parser.add_argument(
        "--headless-config",
        type=argparse.FileType("r"),
        help="run tests in 'headless' mode, using the\
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=[],
        help="module to load tests from",
    )
...
def runHeadless(options):
    ...
    if "enable_smtp" in headless_options and headless_options[
        "enable_smtp"
    ].lower() in ["true", "error"]:
        if (
            headless_options["enable_smtp"].lower() == "true"
            or not results.wasSuccessful()
        ):
            import smtplib
            ...
            server = smtplib.SMTP(
                host=headless_options["smtp_server"], port=headless_options["smtp_port"]
            )
            if (
                "smtp_tls" in headless_options
                and headless_options["smtp_tls"].lower() == "true"
            ):
                server.starttls()
            server.login(
                user=headless_options["smtp_user"],
            ...
            text = (
                "ForceBalance unit test suite ran at %s with %d failures, %d errors. "
                % (
                    time.strftime("%x %X %Z"),
            ...
            if not results.wasSuccessful():
                text += "Error Output:\n"
                with open("/tmp/forcebalance/test.err") as warnings:
                    text += warnings.read()

            log = ""
            with open(logfile) as debug:
                log += debug.read()
