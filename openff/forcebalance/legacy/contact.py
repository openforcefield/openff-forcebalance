...
def atom_distances(xyzlist, atom_contacts, box=None):
    ...
    if box is None:
        ...
        if not box.flags.contiguous:
            ...
            box = np.copy(box)
        _contact_wrap.atomic_contact_rect_image_wrap(
            xyzlist, box, atom_contacts, results
        )
...
def residue_distances(xyzlist, residue_membership, residue_contacts):
    ...
    residue_membership_array = -1 * np.ones(
        (num_residues, max_residue_width), dtype=np.int32
    )
    for i in range(num_residues):
        residue_membership_array[i, 0 : residue_widths[i]] = np.array(
            residue_membership[i], dtype=np.int32
        )

    results = np.zeros((traj_length, num_contacts), np.float32)

    _contact_wrap.closest_contact_wrap(
        xyzlist, residue_membership_array, residue_widths, residue_contacts, results
    )
