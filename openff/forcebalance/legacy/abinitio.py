...
class AbInitio(Target):
    ...
    def __init__(self, options, tgt_opts, forcefield):
        ...
        self.mol = Molecule(
            ...
            top=(
                os.path.join(self.root, self.tgtdir, self.pdb)
                if hasattr(self, "pdb")
                else None
            ),
        )
    ...
    def build_invdist(self, mvals):
        ...
        if len(self.espxyz) > 0:
            invdists = []
            logger.info(
                "\rPreparing the distance matrix... it will have %i * %i * %i = %i elements"
                % (
                    self.ns,
                    self.nesp,
                    self.nparticles,
                    self.ns * self.nesp * self.nparticles,
                )
            )
            sn = 0
            for espset, xyz in zip(self.espxyz, self.mol.xyzs):
                logger.info(
                    "\rGenerating ESP distances for snapshot %i%s\r" % (sn, " " * 50)
                )
                esparr = np.array(espset).reshape(-1, 3)
                # Create a matrix with Nesp rows and Natoms columns.
                DistMat = np.array(
                    [[np.linalg.norm(i - j) for j in xyz] for i in esparr]
                )
                invdists.append(1.0 / (DistMat / bohr2ang))
                sn += 1
        for i in self.pgrad:
            if "VSITE" in self.FF.plist[i]:
    ...
    def compute_netforce_torque(self, xyz, force, QM=False):
        ...
        if self.force_map == "molecule" and "MoleculeNumber" in self.AtomLists:
            ...
            logger.error(
                'force_map keyword "{}" is invalid. Please choose from: {}\n'.format(
                    self.force_map,
                    ", ".join(
                        ['"%s"' % kwds[k] for k in self.AtomLists.keys() if k in kwds]
                    ),
                )
            )
            raise RuntimeError
        ...
        for b in sorted(set(Block)):
            ...
            CrdBlock = np.array(
                list(
                    itertools.chain(*[list(range(3 * i, 3 * i + 3)) for i in AtomBlock])
                )
            )
            com = np.sum(
                xyz1[AtomBlock] * np.outer(Mass[AtomBlock], np.ones(3)), axis=0
            ) / np.sum(Mass[AtomBlock])
    ...
    def read_reference_data(self):
        ...
        if self.force:
            ...
            if isint(self.fitatoms_in):
                if int(self.fitatoms_in) == 0:
                    ...
                    warn_press_key(
                        "Provided an integer for fitatoms; will assume this means the first %i atoms"
                        % int(self.fitatoms_in)
                    )
            ...
            if len(self.fitatoms) > len(self.qmatoms):
                ...
                if self.w_force > 0:
                    if len(self.fitatoms) == len(self.qmatoms):
                        ...
                        logger.info(
                            "Fitting the forces on atoms %s\n"
                            % commadash(self.fitatoms)
                        )
                        logger.info("Pruning the quantum force matrix...\n")
                selct = list(
                    itertools.chain(
                        *[[3 * i + j for j in range(3)] for i in self.fitatoms]
                    )
                )
        ...
        if self.whamboltz == True:
            if self.attenuate:
                ...
                raise RuntimeError
            self.boltz_wts = np.array(
                [
                    float(i.strip())
                    for i in open(
                        os.path.join(self.root, self.tgtdir, "wham-weights.txt")
                    ).readlines()
                ]
            )
            ...
            if os.path.exists(os.path.join(self.root, self.tgtdir, "wham-master.txt")):
                whaminfo = open(
                    os.path.join(self.root, self.tgtdir, "wham-master.txt")
                ).readlines()
                logger.info(
                    "From wham-master.txt, I can see that you're using %i generations\n"
                    % len(whaminfo)
                )
                ...
                for line in whaminfo:
                    ...
                    weight = np.sum(
                        self.boltz_wts[shotcounter : shotcounter + genshots]
                    ) / np.sum(self.boltz_wts)
                    logger.info(
                        " %s, %i snapshots, weight %.3e\n"
        ...
        if self.qmboltz > 0:
            ...
            # Do I need to?
            logboltz = (
                -(self.eqm - np.mean(self.eqm) - self.emd0 + np.mean(self.emd0))
                / kb
                / self.qmboltztemp
            )
            ...
            logger.info(
                "Quantum Boltzmann weights are ON, the formula is exp(-b(E_qm-E_mm)),"
            )
            logger.info(
                "distribution entropy is {:.3f}, equivalent to {:.2f} snapshots\n".format(
                    qmboltzent, np.exp(qmboltzent)
                )
            )
            logger.info(
                "%.1f%% is mixed into the MM boltzmann weights.\n"
                % (self.qmboltz * 100)
            )
        else:
        ...
        if self.use_nft:
            ...
            for i in range(len(self.fqm)):
                self.nftqm.append(
                    self.compute_netforce_torque(self.mol.xyzs[i], self.fqm[i])
                )
    ...
    def indicate(self):
        ...
        if self.force:
            logger.info(
                "Maximum force error on atom %i (%s), frame %i, %8.4f kJ/mol/A\n"
                % (
                    self.maxfatom,
                    self.mol.elem[self.fitatoms[self.maxfatom]],
                    self.maxfshot,
                    self.maxdf / 10,
                )
            )
    ...
    def energy_force_transform(self):
        if self.force:
            ...
            selct = [0] + list(
                itertools.chain(
                    *[[1 + 3 * i + j for j in range(3)] for i in self.fitatoms]
                )
            )
            M = M[:, selct]
            if self.use_nft:
    ...
    def get_energy_force(self, mvals, AGrad=False, AHess=False):
        ...
        if cv and (AGrad or AHess):
            warn_press_key(
                "Covariance is turned on, gradients requested but not implemented (will skip over grad.)"
            )
        Answer = {}
        ...
        if (
            self.w_energy == 0.0
            and self.w_force == 0.0
            and self.w_netforce == 0.0
            and self.w_torque == 0.0
        ):
            AGrad = False
            AHess = False
        Z = 0.0
        ...
        for i in range(NS):
            ...
            if self.force:
                dfrcarray_ = np.array(
                    [
                        np.linalg.norm(
                            M[a + 3 * j : a + 3 * j + 3] - Q[a + 3 * j : a + 3 * j + 3]
                        )
                        for j in range(nat)
                    ]
                )
                ...
                qfrcarray = np.mean(
                    np.array(
                        [
                            np.linalg.norm(Q[a + 3 * j : a + 3 * j + 3])
                            for j in range(nat)
                        ]
                    )
                )
        ...
        if self.writelevel > 0:
            EnergyComparison = np.hstack(
                (col(Q_all_print[:, 0]), col(M_all_print[:, 0]))
            )
        ...
        if self.force and self.writelevel > 1:
            ...
            Dforce_norm = np.array(
                [
                    np.linalg.norm(Mforce_print[i, :] - Qforce_print[i, :])
                    for i in range(NS)
                ]
            )
        ...
        if (
            self.w_energy > 0.0
            or self.w_force > 0.0
            or self.w_netforce > 0.0
            or self.w_torque > 0.0
        ):
        ...
        if cv:
            ...
            MBP = 1 - self.qmboltz
            C = (
                MBP * (QQ_M - np.outer(Q0_M, Q0_M) / Z) / Z
                + QBP * (QQ_Q - np.outer(Q0_Q, Q0_Q) / Y) / Y
            )
            Ci = invert_svd(C)
            ...
            for i in range(1, NCP1):
                ...
                Ci[i, 0] = 0.0
            WCiW = np.array(
                np.matrix(WM) * np.matrix(Ci) * np.matrix(WM)
        ...
        if cv:
            ...
            X2_M = weighted_variance(
                SPiXi, WCiW, Z, X0_M, X0_M, NCP1, subtract_mean=self.savg
            )
            X2_Q = weighted_variance(
                SRiXi, WCiW, Y, X0_Q, X0_Q, NCP1, subtract_mean=self.savg
            )
            ...
            for p in self.pgrad:
                ...
                X2_M_p[p] = weighted_variance(
                    ...
                    M0_M_p[p],
                    NCP1,
                    subtract_mean=self.savg,
                )
                X2_Q_p[p] = weighted_variance(
                    SRiXi_p[p],
                    WCiW,
                    Y,
                    2 * X0_Q,
                ...
                if not AHess:
                    continue
                X2_M_pq[p, p] = weighted_variance2(
                    SPiXi_pq[p, p],
                    WCiW,
                    Z,
                    2 * M0_M_p[p],
                ...
                X2_Q_pq[p, p] = weighted_variance2(
                    SRiXi_pq[p, p],
                    WCiW,
                    Y,
                    2 * M0_Q_p[p],
        ...
        if self.savg:
            E0_M = (
                (2 * Q0_M[0] * M0_M[0] - Q0_M[0] * Q0_M[0] - M0_M[0] * M0_M[0]) / Z / Z
            )
            E0_Q = (
                (2 * Q0_Q[0] * M0_Q[0] - Q0_Q[0] * Q0_Q[0] - M0_Q[0] * M0_Q[0]) / Y / Y
            )
        else:
            E0_M = 0.0
            E0_Q = 0.0
        ...
        if cv:
            dE = MBP * np.sqrt(SPiXi[0, 0] / Z + E0_M) + QBP * np.sqrt(
                SRiXi[0, 0] / Y + E0_Q
            )
        else:
        ...
        dEfrac = MBP * np.sqrt(
            (SPiXi[0] / Z + E0_M) / (QQ_M[0] / Z - Q0_M[0] ** 2 / Z / Z)
        ) + QBP * np.sqrt((SRiXi[0] / Y + E0_Q) / (QQ_Q[0] / Y - Q0_Q[0] ** 2 / Y / Y))
        ...
        if not in_fd():
            ...
            self.e_ctr = MBP * weighted_variance(
                np.array([SPiXi[0]]),
                np.array([WCiW[0]]),
                Z,
                X0_M,
            ...
            self.e_ref = MBP * np.sqrt(
                QQ_M[0] / Z - Q0_M[0] ** 2 / Z / Z
            ) + QBP * np.sqrt(QQ_Q[0] / Y - Q0_Q[0] ** 2 / Y / Y)
            self.e_err = dE
            self.e_err_pct = dEfrac
            if self.force:
                self.f_ctr = MBP * weighted_variance(
                    SPiXi[1 : 1 + 3 * nat],
                    WCiW[1 : 1 + 3 * nat],
                    Z,
                    X0_M,
            ...
            if self.use_nft:
                self.nf_ctr = MBP * weighted_variance(
                    SPiXi[1 + 3 * nat : 1 + 3 * nat + 3 * nnf],
                    WCiW[1 + 3 * nat : 1 + 3 * nat + 3 * nnf],
                    Z,
                    X0_M,
                    X0_M,
                    NCP1,
                ...
                self.tq_ctr = MBP * weighted_variance(
                    SPiXi[1 + 3 * nat + 3 * nnf : 1 + 3 * nat + 3 * nnf + 3 * ntq],
                    WCiW[1 + 3 * nat + 3 * nnf : 1 + 3 * nat + 3 * nnf + 3 * ntq],
                    Z,
                    X0_M,
                    X0_M,
                    NCP1,
                    ...
                    SRiXi[1 + 3 * nat + 3 * nnf : 1 + 3 * nat + 3 * nnf + 3 * ntq],
                    WCiW[1 + 3 * nat + 3 * nnf : 1 + 3 * nat + 3 * nnf + 3 * ntq],
                    Y,
                    X0_Q,
                    X0_Q,
            ...
            pvals = self.FF.make(
                mvals
            )  # Write a force field that isn't perturbed by finite differences.
        if cv:
            Answer = {
                "X": X2,
    ...
    def get_resp(self, mvals, AGrad=False, AHess=False):
        ...
        if AGrad:
            ...
            dqPdqM = np.matrix(
                [
                    (
                        f12d3p(fdwrap(new_charges, mvals, i), h=self.h, f0=charge0)[0]
                        if i in self.pgrad
        ...
        np.array(self.mol.xyzs)
        espqvals = np.array(self.espval)
        np.array(self.espxyz)
        ...
        if AGrad:
            for p in self.pgrad:
                if "VSITE" in self.FF.plist[p]:
                    ddVdqPdVS[p], dddVdqPdVS2[p] = f12d3p(
                        fdwrap(self.build_invdist, mvals, p), h=self.h, f0=self.invdists
                    )
        X = 0
        Q = 0
        D = 0
        ...
        for i in range(self.ns):
            ...
            D += P * (
                np.dot(espqval, espqval) / self.nesp
                - (np.sum(espqval) / self.nesp) ** 2
            )
            if AGrad:
                ...
                if AHess:
                    ...
                    H += (
                        np.array(P * 2 * (dVdqM * dVdqM.T + d2VdqM2 * col(desp)))
                        / self.nesp
                    )
    ...
    def get(self, mvals, AGrad=False, AHess=False):
        ...
        tw = (
            self.w_energy + self.w_force + self.w_netforce + self.w_torque + self.w_resp
        )
        if tw > 0.0:
