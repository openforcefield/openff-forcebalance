class ForceBalanceTestCase:
    def shortDescription(self):
        ...
        message = super().shortDescription()
        if message:
            return message
        else:
            return self.id()

    def assertNdArrayEqual(self, A, B, msg=None, delta=0.00001):
        ...
        if A.shape != B.shape:
            reason = (
                "Tried to compare ndarray of size {} to ndarray of size {}\n".format(
        ...
        if unequal.any():
            ...
            for j, index in enumerate(numpy.argwhere(unequal)):
                ...
                if j >= 4 and n > 9 and n - j > 4:
                    ...
                    reason += "\nA[{}]\t{} =! {}\tB[{}]".format(
                        index[0], A[index[0]], B[index[0]], index[0]
                    )
            reason += "\n"
            if self.longMessage and msg:
                reason += msg
    ...
    def assertEqual(self, first, second, msg=None):
        self.logger.debug(f">ASSERT({str(first)}=={str(second)})\n")
...
class ForceBalanceTestResult(unittest.TestResult):
    ...
    def addFailure(self, test, err):
        ...
        self.logger.warning(
            "\r\x1b[31;1m" + "FAIL" + "\x1b[0m    " + test.shortDescription() + "\n"
        )
    ...
    def addError(self, test, err):
        ...
        self.logger.warning(
            "\r\x1b[33;1mERROR\x1b[0m   " + test.shortDescription() + "\n"
        )
...
class ForceBalanceTestRunner:
    ...
    def __init__(
        self, logger=forcebalance.output.getLogger("forcebalance.test"), verbose=False
    ):
        self.logger = logger
    ...
    def check(self, test_modules=__all__):
        ...
        if __name__ == "forcebalance.test.__init__":
            test_modules = [
                "forcebalance.test." + test_module for test_module in test_modules
            ]
            ...
            for module in suite:
                for test in module:
                    ...
                    if modName in test_modules:
                        if modName == "test_system":
                            systemTests.addTest(test)
                        else:
                            tests.addTest(test)
