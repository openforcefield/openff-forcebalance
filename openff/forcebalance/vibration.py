...
class Vibration(Target):
    ...
    def indicate(self):
        ...
        if self.reassign == "overlap":
            count_assignment(self.c2r)
        ...
        data = OrderedDict(
            [
                (
                    i,
                    [
                        "%.4f" % self.ref_eigvals[i],
                        "%.4f" % self.calc_eigvals[i],
                        "%.4f" % (self.calc_eigvals[i] - self.ref_eigvals[i]),
                        "%.4f" % self.overlaps[i],
                    ],
                )
    ...
    def get(self, mvals, AGrad=False, AHess=False):
        ...
        def get_eigvals(mvals_):
            ...
            if self.reassign in ["permute", "overlap"]:
                ...
                a = np.array(
                    [
                        [(1.0 - vib_overlap(self.engine, v1, v2)) for v2 in eigvecs]
                        for v1 in self.ref_eigvecs
                    ]
                )
                a += dev
        ...
        for p in self.pgrad:
            Answer["G"][p] = (
                ...
                * np.dot(D, dV[p, :])
                / self.denom**2
                / (len(D) if self.normalize else 1)
            )
            for q in self.pgrad:
                Answer["H"][p, q] = (
                    2
...
def vib_overlap(engine, v1, v2):
    ...
    """
    if hasattr(engine, "realAtomIdxs"):
        realAtoms = engine.realAtomIdxs
    else:
        realAtoms = [i for i in range(len(engine.AtomLists["Mass"]))]
