        ...
        if re.match(r"^\$", line):
            ...
            if word == "END":  # End of a section, time to reinitialize variables.
                if section == "GLOBAL":
                    ...
                    warn_press_key(
                        "Encountered $end for unsupported section %s on line %i"
                        % (word, ln)
                    )
                section = "NONE"
            elif section == "NONE":
                ...
                warn_press_key(
                    "Encountered section keyword {} when already in section {}".format(
                        word, section
                    )
                )
        elif section == "GLOBAL":
            if key in ["keyfile", "energy_unit"]:
                ...
                warn_press_key(
                    "Encountered unsupported key %s in section %s on line %i"
                    % (key, section, ln)
                )
        elif section == "SYSTEM":
            if key == "name":
...
class BindingEnergy(Target):
    ...
    def __init__(self, options, tgt_opts, forcefield):
        ...
        self.set_option(
            None, None, "inter_txt", os.path.join(self.tgtdir, tgt_opts["inter_txt"])
        )
        self.global_opts, self.sys_opts, self.inter_opts = parse_interactions(
            self.inter_txt
        )
        ...
        for inter in self.inter_opts:
            if "energy_unit" in self.inter_opts[inter] and self.inter_opts[inter][
                "energy_unit"
            ].lower() not in ["kilocalorie_per_mole", "kilocalories_per_mole"]:
                logger.error(
                    "Usage of physical units is has been removed, please provide all binding energies in kcal/mole\n"
                )
                raise RuntimeError
            self.inter_opts[inter]["reference_physical"] = self.inter_opts[inter][
                "energy"
            ]
        ...
        if tgt_opts["energy_denom"] == 0.0:
            self.set_option(
                ...
                val=np.std(
                    np.array(
                        [val["reference_physical"] for val in self.inter_opts.values()]
                    )
                ),
            )
        else:
        ...
        for sysname, sysopt in self.sys_opts.items():
            ...
            self.engines[sysname] = self.engine_(
                ...
                mol=M,
                name=sysname,
                tinker_key=os.path.join(sysopt["keyfile"]),
                **engine_args
            )

    def system_driver(self, sysname):
        opts = self.sys_opts[sysname]
        return self.engines[sysname].energy_rmsd(
    ...
    def indicate(self):
        printcool_dictionary(
            self.PrintDict,
            title="Interaction Energies (kcal/mol), Objective = % .5e\n %-20s %9s %9s %9s %11s"
            % (self.energy_part, "Interaction", "Calc.", "Ref.", "Delta", "Term"),
        )
        if len(self.RMSDDict) > 0:
            printcool_dictionary(
                self.RMSDDict,
                title="Geometry Optimized Systems (Angstrom), Objective = %.5e\n %-38s %11s %11s"
    ...
    def get(self, mvals, AGrad=False, AHess=False):
        ...
        def compute(mvals_):
            ...
            for inter_ in self.inter_opts:
                ...
                w_ = (
                    self.inter_opts[inter_]["weight"]
                    if "weight" in self.inter_opts[inter_]
                    else 1.0
                )
