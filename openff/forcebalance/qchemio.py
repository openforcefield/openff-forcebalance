...
pdict = {"BASS": {0: "A", 1: "C"}, "BASSP": {0: "A", 1: "B", 2: "C"}}
    ...
    def feed(self, line):
        ...
        if match(r"^\$", line):
            ...
            if match("^[A-Za-z][a-z]* +0$", line):
                ...
                self.cnum += 1
                logger.info(f"{line} {str(self.snum)} {str(self.cnum)}\n")
...
def QChem_Dielectric_Energy(fnm, wq):
    ...
    for i in range(QCIn.ns):
        ...
        queue_up(
            wq,
            "qchem40 qchem.in qchem.out",
            input_files=["qchem.in"],
            output_files=["qchem.out"],
            verbose=False,
        )
        Leave(sdir)
