...
class Thermo(Target):
    ...
    def __init__(self, options, tgt_opts, forcefield):
        ...
        self._read_expdata(os.path.join(self.root, self.tgtdir, self.expdata_txt))
    ...
    def _read_expdata(self, expdata):
        ...
        while line:
            ...
            if "=" in line:  # Read variable
                ...
                pressure = None if vals[-1].lower() == "none" else float(vals[-1])
                ...
                foundHeader = True
                headers = list(
                    zip(*[tuple(h.split("_")) for h in line.split() if h != "w"])
                )
    ...
    def submit_jobs(self, mvals, AGrad=True, AHess=True):
        ...
        for pt in self.points:
            ...
            link_dir_contents(
                os.path.join(self.root, self.tgtdir, str(pt.idnr)), os.getcwd()
            )
    ...
    def indicate(self):
        ...
        def print_item(key, physunit):
            if self.Xp[key] > 0:
                the_title = (
                    f"{self.name} {key.capitalize()} ({physunit})\n"
                ...
                printcool_dictionary(
                    self.Pp[key], title=the_title, bold=True, color=4, keywidth=15
                )

                bar = printcool(
                    "%s objective function: % .3f%s"
                    % (key.capitalize(), self.Xp[key], ", Derivative:" if AGrad else "")
                )
                if AGrad:
                    self.FF.print_map(vals=self.Gp[key])
                    logger.info(bar)

                PrintDict[key] = "{: 10.5f} {: 8.3f} {: 14.5e}".format(
                    self.Xp[key],
                    self.Wp[key],
    ...
    def objective_term(self, quantity):
        ...
        printer = OrderedDict(
            [
                (
                    "    %-5d %-12.2f %-8.1f" % (pt.idnr, pt.temperature, pt.pressure),
                    (
    ...
    def get(self, mvals, AGrad=True, AHess=True):
        ...
        Objective = np.average(
            Xs, weights=(None if np.all(reweighted == 0) else reweighted), axis=0
        )
        if AGrad:
            Gradient = np.average(
                Gs, weights=(None if np.all(reweighted == 0) else reweighted), axis=0
            )
        if AHess:
            Hessian = np.average(
                Hs, weights=(None if np.all(reweighted == 0) else reweighted), axis=0
            )
        ...
        if not in_fd():
            ...
            self.Wp = {
                q: reweighted[self.quantities.index(q)] for (q, dic) in obj.items()
            }
