...
from openff.forcebalance.leastsq import CheckBasis, LeastSquares
...
class GBS_Reader(BaseReader):
    ...
    def build_pid(self, pfld):
        ...
        return (
            ptype
            + ":"
            + "Elem=%s,AMom=%s,Bas=%i,Con=%i"
            % (
                self.element,
                self.amom,
                self.basis_number[self.element],
                self.contraction_number,
            )
        )
...
class THCDF_Psi4(LeastSquares):
    ...
    def indicate(self):
        ...
        logger.info(
            "\rTarget: %-15s Molecules = %-30s %s"
            % (
                self.name,
                str(self.Molecules),
                "Mean (Max) Error: {:8.4f}% ({:8.4f}%) Energies: DF {:+.3e} MP2 {:+.3e} Delta {: .3e} Objective = {:.5e}".format(
                    100 * MAD / self.MAQ,
                    100 * np.max(np.abs(self.D)) / self.MAQ,
                    self.DF_Energy,
                    self.MP2_Energy,
                    self.DF_Energy - self.MP2_Energy,
                    self.objective,
                ),
            )
        )
    ...
    def driver(self):
        ...
        if not in_fd() and CheckBasis():
            ...
            _exec(f"cp {self.GBSfnm} {self.GBSfnm}.bak", print_command=False)
            ...
            for ln, line in enumerate(open(self.GBSfnm).readlines()):
                ...
                if FK.isdata and key in LI_lines:
                    if LI_lines[key][1]:
                        ...
                        for p_destroy in [
                            i
                            for i, fld in enumerate(self.FF.pfields)
                            if any(
                                [
                                    subfld[0] == self.GBSfnm and subfld[1] == ln0[ln]
                                    for subfld in fld
                                ]
                            )
                        ]:
            ...
            for line in FK_lines:
                print(line, end=" ", file=o)
            o.close()
            _exec(f"cp {self.GBSfnm}.bak {self.GBSfnm}", print_command=False)

            if (
                len(
                    list(
                        itertools.chain(
                            *(self.FF.linedestroy_save + [self.FF.linedestroy_this])
                        )
                    )
                )
                > 0
...
class RDVR3_Psi4(Target):
    ...
    def submit_jobs(self, mvals, AGrad=True, AHess=True):
        ...
        def submit_psi(this_apath, dname, these_mvals):
            ...
            cwd = os.getcwd()
            if not os.path.exists(this_apath):
                os.makedirs(this_apath)
            os.chdir(this_apath)
            ...
            if wq is None:
                ...
                input_files += [
                    (
                        os.path.join(self.root, self.tgtdir, dname, "build.dat"),
                        "build.dat",
                    )
                ]
                input_files += [
    ...
    def get(self, mvals, AGrad=False, AHess=False):
        ...
        def f2d4p(f, h, f0=None):
            if f0 is None:
                fpp, fp0, f0p, f0 = (
                    f(i * h, j * h) for i, j in [(1, 1), (1, 0), (0, 1), (0, 0)]
                )
            else:
        ...
        for d in self.objfiles:
            ...
            x = (
                float(
                    open(os.path.join(apath, "objective.out")).readlines()[0].split()[1]
                )
                * self.factor
            )
            for p in range(self.FF.np):
                if self.callderivs[d][p]:
                    ...
                    if AHess:
                        if wq is not None:
                            ...
                            grad[p], hdiag[p] = f12d3p(
                                fdwrap(reader, mvals, p, h=self.h), h=self.h, f0=x
                            )
                        else:
                            grad[p], hdiag[p] = f12d3p(
                                fdwrap(self.driver, mvals, p, d=d), h=self.h, f0=x
                            )
                        hess[p, p] = hdiag[p]
                    elif AGrad:
                        if self.bidirect:
