...
class Counterpoise(Target):
    ...
    def __init__(self, options, tgt_opts, forcefield):
        ...
        self.elem, self.xyzs = self.loadxyz(
            os.path.join(self.root, self.tgtdir, "all.xyz")
        )
    ...
    def get(self, mvals, AGrad=False, AHess=False):
        ...
        for s in range(self.ns):
            ...
            for i in range(self.na):
                for j in range(i + 1, self.na):
                    ...
                    ai = self.elem[i]
                    aj = self.elem[j]
                    aiaj = ai < aj and f"{ai}{aj}" or f"{aj}{ai}"
                    ...
                    if all([k + aiaj in self.FF.map for k in pidlist]):
                        ...
                        A, B, C = (
                            self.FF.pvals[self.FF.map[k + aiaj]] for k in pidlist
                        )
        ...
        with wopen("results") as f:
            f.writelines(
                [f"{cpmm[i]: .4f} {self.cpqm[i]: .4f}\n" for i in range(len(cpmm))]
            )
