...
def is_mol2_atom(line):
    ...
    if len(s) < 9:
        return False
    return all(
        [isint(s[0]), isfloat(s[2]), isfloat(s[3]), isfloat(s[4]), isfloat(s[8])]
    )


def write_leap(
    fnm, mol2=[], frcmod=[], pdb=None, prefix="amber", spath=[], delcheck=False
):
    ...
    for i in frcmod:
        if i not in have_fmod:
            warn_press_key(f"WARNING: {i} is not being loaded in {fnm}")
    for i in mol2:
        if i not in have_mol2:
            warn_press_key(f"WARNING: {i} is not being loaded in {fnm}")

    fout = fnm + "_"
    line_out.append(f"saveamberparm {ambername} {prefix}.prmtop {prefix}.inpcrd\n")
    line_out.append("quit\n")
    if os.path.exists(fout):
        os.remove(fout)
    with wopen(fout) as f:
        print("".join(line_out), file=f)
...
def write_mdin(
    ...
    cntrl_vars["imin"] = cntrl_var(
        name="imin",
        value={"min": "1", "eq": "0", "md": "0", "sp": "5"},
        comment="0 = MD; 1 = Minimize; 5 = Trajectory analysis",
        priority=1,
    )
    # Options pertaining to minimization
    cntrl_vars["ntmin"] = cntrl_var(
        name="ntmin",
        value={"min": "2"},
        comment="Minimization algorithm; 2 = Steepest descent",
        priority=3,
    )
    cntrl_vars["dx0"] = cntrl_var(
        name="dx0", value={"min": "0.1"}, comment="Minimizer step length", priority=3
    )
    cntrl_vars["maxcyc"] = cntrl_var(
        name="maxcyc",
        value={"min": "%i" % nsteps, "sp": "1"},
        comment="Number of minimization steps",
        priority=3,
    )
    # MD options - time step and number of steps
    cntrl_vars["dt"] = cntrl_var(
        name="dt",
        value={"eq": "%.8f" % timestep, "md": "%.8f" % timestep},
        comment="Time step (ps)",
        priority=2,
    )
    cntrl_vars["nstlim"] = cntrl_var(
        name="nstlim",
        value={"eq": "%i" % nsteps, "md": "%i" % nsteps},
        comment="Number of MD steps",
        priority=2,
    )
    # ntpr, ntwx and ntwr for eq and md runs should be set by this function.
    cntrl_vars["ntpr"] = cntrl_var(
        name="ntpr",
        value={"min": "%i" % nsave, "eq": "%i" % nsave, "md": "%i" % nsave},
        comment="Interval for printing output",
        priority={"min": 1, "eq": 2, "md": 2, "sp": 1},
    )
    cntrl_vars["ntwx"] = cntrl_var(
        name="ntwx",
        value={"min": "%i" % nsave, "eq": "%i" % nsave, "md": "%i" % nsave},
        comment="Interval for writing trajectory",
        priority={"min": 1, "eq": 2, "md": 2, "sp": 1},
    )
    cntrl_vars["ntwr"] = cntrl_var(
        name="ntwr",
        value={"min": "%i" % nsave, "eq": "%i" % nsteps, "md": "%i" % nsteps},
        comment="Interval for writing restart",
        priority={"min": 1, "eq": 1, "md": 1, "sp": 1},
    )
    cntrl_vars["ntwv"] = cntrl_var(
        name="ntwv",
        value={"md": "-1"},
        comment="Interval for writing velocities",
        priority={"min": 1, "eq": 1, "md": 2, "sp": 1},
    )
    cntrl_vars["ntwe"] = cntrl_var(
        name="ntwe",
        value={"md": "%i" % nsave},
        comment="Interval for writing energies (disabled)",
        priority=1,
    )
    cntrl_vars["nscm"] = cntrl_var(
        name="nscm",
        value={"eq": "1000", "md": "1000"},
        comment="Interval for removing COM translation/rotation",
        priority=3,
    )
    # Insist on NetCDF trajectories for ntxo, ioutfm
    cntrl_vars["ntxo"] = cntrl_var(
        name="ntxo",
        value={"min": "2", "eq": "2", "md": "2"},
        comment="Restart output format; 1 = ASCII, 2 = NetCDF",
        priority=1,
    )
    cntrl_vars["ioutfm"] = cntrl_var(
        name="ioutfm",
        value={"min": "1", "eq": "1", "md": "1"},
        comment="Trajectory format; 0 = ASCII, 1 = NetCDF",
        priority=1,
    )
    # min and eq read coors only; md is a full restart
    cntrl_vars["ntx"] = cntrl_var(
        name="ntx",
        value={"min": "1", "eq": "1", "md": "5"},
        comment="1 = Read coors only; 5 = Full restart",
        priority=1,
    )
    cntrl_vars["irest"] = cntrl_var(
        name="irest",
        value={"min": "0", "eq": "0", "md": "1"},
        comment="0 = Do not restart ; 1 = Restart",
        priority=1,
    )
    ...
    if pbc:
        ...
        cntrl_vars["cut"] = cntrl_var(
            name="cut",
            value={"min": "8.0", "eq": "8.0", "md": "8.0", "sp": "8.0"},
            comment="Nonbonded cutoff",
            priority=3,
        )
        cntrl_vars["ntb"] = cntrl_var(
            name="ntb",
            value={"min": "1", "eq": ntb_eqmd, "md": ntb_eqmd, "sp": ntb_eqmd},
            comment="0 = no PBC ; 1 = constant V ; 2 = constant P",
            priority=1,
        )
        cntrl_vars["ntp"] = cntrl_var(
            name="ntp",
            value={"min": "0", "eq": ntp_eqmd, "md": ntp_eqmd, "sp": ntp_eqmd},
            comment="0 = constant V ; 1 = isotropic scaling",
            priority=1,
        )
        cntrl_vars["iwrap"] = cntrl_var(
            name="iwrap",
            value={"min": "1", "eq": "1", "md": "1"},
            comment="Wrap molecules back into box",
            priority=3,
        )
        cntrl_vars["igb"] = cntrl_var(
            name="igb",
            value={"min": "0", "eq": "0", "md": "0", "sp": "0"},
            comment="0 = No generalized Born model",
            priority=3,
        )
        if pressure is not None:
            # We should use Berendsen for equilibration and MC for production.
            cntrl_vars["barostat"] = cntrl_var(
                name="barostat",
                value={"eq": "1", "md": "2"},
                comment="1 = Berendsen; 2 = Monte Carlo",
                priority=1,
            )
            cntrl_vars["mcbarint"] = cntrl_var(
                name="mcbarint",
                value={"md": "25"},
                comment="MC barostat rescaling interval",
                priority=3,
            )
        else:
        ...
        cntrl_vars["cut"] = cntrl_var(
            name="cut",
            value={"min": "9999.0", "eq": "9999.0", "md": "9999.0", "sp": "9999.0"},
            comment="Nonbonded cutoff",
            priority=1,
        )
        cntrl_vars["ntb"] = cntrl_var(
        ...
        cntrl_vars["igb"] = cntrl_var(
            name="igb",
            value={"min": "6", "eq": "6", "md": "6", "sp": "6"},
            comment="6 = Vacuum phase simulation",
            priority=1,
        )
        cntrl_vars["iwrap"] = cntrl_var(
            name="iwrap", value={}, comment="Wrap molecules back into box", priority=1
        )
        cntrl_vars["barostat"] = cntrl_var(
            name="barostat",
            value={},
    ...
    if temperature is not None:
        cntrl_vars["tempi"] = cntrl_var(
            name="tempi",
            value={"eq": "%i" % temperature, "md": "%i" % temperature},
            comment="Initial temperature",
            priority=1,
        )
        cntrl_vars["temp0"] = cntrl_var(
            name="temp0",
            value={"eq": "%i" % temperature, "md": "%i" % temperature},
            comment="Reference temperature",
            priority=1,
        )
        cntrl_vars["ntt"] = cntrl_var(
            name="ntt",
            value={"eq": "3", "md": "3"},
            comment="Thermostat ; 3 = Langevin",
            priority=1,
        )
        cntrl_vars["gamma_ln"] = cntrl_var(
            name="gamma_ln",
            value={"eq": "1.0", "md": "1.0"},
            comment="Langevin collision frequency (ps^-1)",
            priority=3,
        )
    else:
        cntrl_vars["tempi"] = cntrl_var(
            name="tempi", value={}, comment="Initial temperature", priority=1
        )
        cntrl_vars["temp0"] = cntrl_var(
            name="temp0", value={}, comment="Reference temperature", priority=1
        )
        cntrl_vars["ntt"] = cntrl_var(
            name="ntt", value={}, comment="Thermostat ; 3 = Langevin", priority=1
        )
        cntrl_vars["gamma_ln"] = cntrl_var(
            name="gamma_ln",
            value={},
    ...
    cntrl_vars["ntc"] = cntrl_var(
        name="ntc",
        value={"min": "1", "eq": "1", "md": "1", "sp": "1"},
        comment="SHAKE; 1 = none, 2 = H-bonds, 3 = All-bonds",
        priority={"min": 1, "eq": 3, "md": 3, "sp": 1},
    )
    cntrl_vars["ntf"] = cntrl_var(
        name="ntf",
        value={"min": "1", "eq": "1", "md": "1", "sp": "1"},
        comment="No bonds involving H-atoms (use with NTC=2)",
        priority={"min": 1, "eq": 3, "md": 3, "sp": 1},
    )
    cntrl_vars["tol"] = cntrl_var(
        name="tol", value={}, comment="SHAKE tolerance,", priority=4
    )
    # Random number seed for equilibration and dynamics
    cntrl_vars["ig"] = cntrl_var(
        name="ig",
        value={"eq": "-1", "md": "-1"},
        comment="Random number seed; -1 based on date/time",
        priority=3,
    )
        ...
        with open(fout, "w") as f:
            ...
            for name, block_dict, suffix in zip(names, block_dicts, suffixes):
                ...
                for key, val in block_dict.items():
                    print(
                        "%-20s ! %s" % (f"{key}={val},", cntrl_comm[key]),
                        file=f,
                    )
                print("/", file=f)
                for line in suffix:
    ...
    def feed(self, line):
        ...
        if self.itype in self.pdict:
            if "Atom" in self.pdict[self.itype] and match(" *[0-9]", line):
                ...
                self.molatom = (
                    self.mol,
                    self.atom if type(self.atom) is list else [self.atom],
                )
    ...
    def feed(self, line):
        ...
        if self.itype in self.pdict:
            if "Atom" in self.pdict[self.itype] and self.haveAtomLine:
                # List the atoms in the interaction.
                self.atom = [
                    s[i].replace(" -", "-") for i in self.pdict[self.itype]["Atom"]
                ]
...
class PrmtopLoader:
    ...
    def __init__(self, inFilename):
        ...
        for line in fIn:
            if line.startswith("%VERSION"):
                ...
                self._raw_format[self._flags[-1]] = (
                    format,
                    m.group(1),
                    m.group(2),
                    m.group(3),
    ...
    def getAngles(self):
        ...
        anglePointers = (
            self._raw_data["ANGLES_INC_HYDROGEN"]
    ...
    def getDihedrals(self):
        ...
        dihedralPointers = (
            self._raw_data["DIHEDRALS_INC_HYDROGEN"]
    ...
    def getGBParms(self, symbls=None):
        ...
        for iAtom in range(len(radii)):
            self._gb_List.append(
                (float(radii[iAtom]) * lengthConversionFactor, float(screen[iAtom]))
            )
        return self._gb_List
...
class AMBER(Engine):
    ...
    def prepare(self, pbc=False, **kwargs):
        ...
        if (
            hasattr(self, "target")
            and hasattr(self.target, "loop_over_snapshots")
    ...
    def leap(self, read_prmtop=False, count_mols=False, name=None, delcheck=False):
        ...
        if read_prmtop:
            ...
            self.AtomLists["ResidueNumber"] = [
                prmtop.getResidueNumber(i) for i in range(na)
            ]
            self.AtomLists["ResidueName"] = [
                prmtop.getResidueLabel(i) for i in range(na)
            ]
            # AMBER virtual sites don't have to have mass zero; this is my
            # best guess at figuring out where they are.
            self.AtomMask = [
                self.AtomLists["Mass"][i] >= 1.0 or self.AtomLists["Name"][i] != "LP"
                for i in range(na)
            ]
            if self.pbc != prmtop.getIfBox():
                raise RuntimeError(
                    "Engine was created with pbc = %i but prmtop.getIfBox() = %i"
                    % (self.pbc, prmtop.getIfBox())
                )
    ...
    def evaluate_sander(self, leap=True, traj_fnm=None, snapshots=None):
        ...
        if "ntc" in cntrl_vars:
            inp.ntc = int(cntrl_vars["ntc"])
        if "ntf" in cntrl_vars:
            inp.ntf = int(cntrl_vars["ntf"])
        if "cut" in cntrl_vars:
            inp.cut = float(cntrl_vars["cut"])
    ...
    def evaluate_cpptraj(self, leap=True, traj_fnm=None, potential=False):
        ...
        if potential:
            ...
            potentials = (
                np.array([float(line.split()[ie]) for line in esander_lines[1:]])
                * 4.184
            )
        ...
        dipoles = (
            np.array(
                [
                    [float(w) for w in line.split()[1:4]]
                    for line in list(open("dipoles.txt").readlines())[1:]
                ]
            )
            / 0.20819434
        )
        ...
        if self.pbc:
            volumes = (
                np.array(
                    [
                        float(line.split()[1])
                        for line in list(open("volumes.txt").readlines())[1:]
                    ]
                )
                / 1000
            )
    ...
    def kineticE_cpptraj(self, leap=False, traj_fnm=None):
        ...
        temps = np.array(
            [
                float(line.split()[1])
                for line in list(open("temperature.txt").readlines())[1:]
            ]
        )
        dof = 3 * self.natoms
        kinetics = 4.184 * kb_kcal * dof * temps / 2.0
        print(
            "Average temperature is {:.2f}, kinetic energy {:.2f}".format(
                np.mean(temps), np.mean(kinetics)
            )
        )
    ...
    def molecular_dynamics(
        ...
        **kwargs,
        ...
        if minimize:
            ...
            if verbose:
                printcool("Minimizing the energy", color=0)
            write_mdin(
                "min", "%s-min.mdin" % self.name, pbc=self.pbc, mdin_orig=self.mdin
            )
            self.leap(read_prmtop=False, count_mols=False, delcheck=True)
            self.callamber(
        ...
        if nequil > 0:
            ...
            if verbose:
                printcool("Running equilibration dynamics", color=0)
            self.callamber(
        ...
        # Run production.
        if verbose:
            printcool("Running production dynamics", color=0)
        write_mdin(
            "md",
            "%s-md.mdin" % self.name,
            nsteps=nsteps,
 ...
 /
"""
            with wopen("%s-nr.in" % self.name) as f:
                print(opt_temp.format(), file=f)
            self.callamber(
                "nmode -O -i {}-nr.in -c {}.inpcrd -p {}.prmtop -r {}.rst -o {}-nr.out".format(
 ...
 /
"""
        with wopen("%s-nmode.in" % self.name) as f:
            print(nmode_temp.format(nvect=3 * self.mol.na), file=f)
        self.callamber(
            "nmode -O -i {}-nmode.in -c {}.rst -p {}.prmtop -v {}-vecs.out -o {}-vibs.out".format(
                self.name, self.name, self.name, self.name, self.name
            )
        )
...
class Liquid_AMBER(Liquid):
    ...
    def __init__(self, options, tgt_opts, forcefield):
        ...
        for f in [self.liquid_coords, self.gas_coords]:
            if os.path.exists(
                os.path.join(self.root, self.tgtdir, "%s.mdin" % os.path.splitext(f)[0])
            ):
                self.nptfiles.append("%s.mdin" % os.path.splitext(f)[0])
                if f == self.gas_coords:
                    self.gas_engine_args["mdin"] = os.path.splitext(f)[0]
        for mol2 in listfiles(
            None, "mol2", err=False, dnm=os.path.join(self.root, self.tgtdir)
        ):
            self.nptfiles.append(mol2)
        for frcmod in listfiles(
            None, "frcmod", err=False, dnm=os.path.join(self.root, self.tgtdir)
        ):
