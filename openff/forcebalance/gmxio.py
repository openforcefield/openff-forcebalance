...
from openff.forcebalance.lipid import Lipid
...
def edit_mdp(fin=None, fout=None, options={}, defaults={}, verbose=False):
    ...
    options = OrderedDict(
        [
            (key.lower().replace("-", "_"), str(val) if val is not None else None)
        ...
        for line in open(fin).readlines():
            ...
            if key in options:
                ...
                if key in clashes and val != val0:
                    logger.error(
                        "edit_mdp tried to set {} = {} but its original value was {} = {}\n".format(
                            key, val, key, val0
                        )
                    )
                    raise RuntimeError
    ...
    if fout != None:
        ...
        file_out.close()
    if verbose:
        printcool_dictionary(options, title=f"{fin} -> {fout} with options:")
    return all_options
...
class ITP_Reader(BaseReader):
    ...
    def feed(self, line):
        ...
        if re.match(r"^ *\[.*\]", line):
            ...
            self.AtomTypes[atype["atomtype"]] = {
                "AtomClass": atype["batomtype"],
            ...
            if (
                self.itype in ["PDIHS", "PIMPDIHS", "FOURDIHS", "PDIHMULS"]
                and len(s) >= 7
            ):
...
class GMX(Engine):
    ...
    def prepare(self, pbc=False, **kwargs):
        ...
        if "constraints" in mdp_dict.keys() and mdp_dict["constraints"].replace(
            "-", "_"
        ).lower() in ["h_bonds", "all_bonds", "h_angles", "all_angles"]:
        ...
        if hasattr(self, "FF"):
            ...
            if self.top is not None and os.path.exists(self.top):
                if self.top not in self.FF.fnms and (
                    not any(
                        [
                            any([fnm in line for fnm in self.FF.fnms])
                            for line in open(self.top)
                        ]
                    )
                ):
                    logger.warning(
        ...
        edit_mdp(
            fin=self.mdp,
            fout="%s.mdp" % self.name,
            options=gmx_opts,
            defaults=self.gmx_defs,
        )

        ## Call grompp followed by gmxdump to read the trajectory
        o = self.warngmx(
            "grompp -c {}.gro -p {}.top -f {}.mdp -o {}.tpr".format(
        ...
        for line in o:
            ...
            if "cgs[" in line:
                ...
                for i in range(ai[1] - ai[0] + 1):
                    self.AtomLists["ChargeGroupNumber"].append(cg)
            if "mols[" in line:
                ai = [int(i) for i in line.split("{")[1].split("}")[0].split("..")]
                mn = int(line.split("[")[1].split("]")[0])
                for i in range(ai[1] - ai[0] + 1):
                    self.AtomLists["MoleculeNumber"].append(mn)
        os.unlink("mdout.mdp")
    ...
    def warngmx(self, command, warnings=[], maxwarn=1, **kwargs):
        ...
        command = " ".join(csplit)
        o = self.callgmx(
            command, persist=True, copy_stderr=True, print_error=False, **kwargs
        )
        warnthis = []
        fatal = 0
        warn = 0
    ...
    def optimize(self, shot, crit=1e-4, align=True, **kwargs):
        ...
        edit_mdp(
            fin="%s.mdp" % self.name, fout="%s-min.mdp" % self.name, options=min_opts
        )

        self.warngmx(
            "grompp -c {}.gro -p {}.top -f {}-min.mdp -o {}-min.tpr".format(
                self.name, self.name, self.name, self.name
            )
        )
        self.callgmx("mdrun -deffnm %s-min -nt 1" % self.name)
        self.callgmx(
            "trjconv -f {}-min.trr -s {}-min.tpr -o {}-min.gro -ndec 9".format(
                self.name, self.name, self.name
            ),
            stdin="System",
        )
        self.callgmx(
            "g_energy -xvg no -f {}-min.edr -o {}-min-e.xvg".format(
                self.name, self.name
            ),
            stdin="Potential",
        )
        ...
        M = Molecule("%s.gro" % self.name, build_topology=False) + Molecule(
            "%s-min.gro" % self.name, build_topology=False
        )
        if not self.pbc:
    ...
    def evaluate_(self, force=False, dipole=False, traj=None):
        ...
        shot_opts = OrderedDict(
            [("nsteps", 0), ("nstxout", 0), ("nstxtcout", 0), ("nstenergy", 1)]
        )
        shot_opts["nstfout"] = 1 if force else 0
        edit_mdp(
            fin="%s.mdp" % self.name, fout="%s-1.mdp" % self.name, options=shot_opts
        )

        ## Call grompp followed by mdrun.
        self.warngmx(
            "grompp -c {}.gro -p {}.top -f {}-1.mdp -o {}.tpr".format(
                self.name, self.name, self.name, self.name
            )
        )
        self.callgmx(
            "mdrun -deffnm {} -nt 1 -rerunvsite {}".format(
                self.name, "-rerun %s" % traj if traj else ""
            )
        )
        ...
        self.callgmx(
            f"g_energy -xvg no -f {self.name}.edr -o {self.name}-e.xvg",
            stdin="Potential",
        )
        ...
        if force:
            self.callgmx(
                "g_traj -xvg no -s {}.tpr -f {}.trr -of {}-f.xvg -fp".format(
                    self.name, self.name, self.name
                ),
                stdin="System",
            )
            Result["Force"] = np.array(
                [
                    [
                        float(j)
        ...
        if dipole:
            self.callgmx(
                "g_dipoles -s {}.tpr -f {} -o {}-d.xvg -xvg no".format(
                    self.name, traj if traj else "%s.gro" % self.name, self.name
                ),
                stdin="System\n",
            )
            Result["Dipole"] = np.array(
                [
                    [float(i) for i in line.split()[1:4]]
                    for line in open("%s-d.xvg" % self.name)
                ]
            )
    ...
    def make_gro_trajectory(self, fout=None):
        ...
        self.callgmx(
            "trjconv -f {} -o {} -ndec 9 -pbc mol -novel -noforce".format(
                self.mdtraj, fout
            ),
            stdin="System",
        )
    ...
    def energy_rmsd(self, shot, optimize=True):
        ...
        if optimize:
            ...
            self.mol[shot].write("%s.gro" % self.name)
            self.warngmx(
                "grompp -c {}.gro -p {}.top -f {}.mdp -o {}-1.tpr".format(
                    self.name, self.name, self.name, self.name
                )
            )
            self.callgmx(f"mdrun -deffnm {self.name}-1 -nt 1 -rerunvsite")
            self.callgmx(
                "g_energy -xvg no -f {}-1.edr -o {}-1-e.xvg".format(
                    self.name, self.name
                ),
                stdin="Potential",
            )
    ...
    def interaction_energy(self, fraga, fragb):
        ...
        write_ndx(
            "%s.ndx" % self.name,
            OrderedDict([("A", [i + 1 for i in fraga]), ("B", [i + 1 for i in fragb])]),
        )

        ## .mdp files for fully interacting and interaction-excluded systems.
        edit_mdp(
            fin="%s.mdp" % self.name,
            fout="%s-i.mdp" % self.name,
            options={"xtc_grps": "A B", "energygrps": "A B"},
        )
        edit_mdp(
            fin="%s.mdp" % self.name,
            fout="%s-x.mdp" % self.name,
            options={"xtc_grps": "A B", "energygrps": "A B", "energygrp-excl": "A B"},
        )
        ...
        self.callgmx(
            "mdrun -deffnm {}-i -nt 1 -rerunvsite -rerun {}-all.gro".format(
                self.name, self.name
            )
        )
        self.callgmx(
            f"g_energy -f {self.name}-i.edr -o {self.name}-i-e.xvg -xvg no",
            stdin="Potential\n",
        )
        I = []
        ...
        self.callgmx(
            "mdrun -deffnm {}-x -nt 1 -rerunvsite -rerun {}-all.gro".format(
                self.name, self.name
            )
        )
        self.callgmx(
            f"g_energy -f {self.name}-x.edr -o {self.name}-x-e.xvg -xvg no",
            stdin="Potential\n",
        )
        X = []
    ...
    def normal_modes(self, shot=0, optimize=True):
        ...
        edit_mdp(
            fin="%s.mdp" % self.name,
            fout="%s-nm.mdp" % self.name,
            options={"integrator": "nm"},
        )

        if optimize:
            self.optimize(shot)
            self.warngmx(
                "grompp -c {}-min.gro -p {}.top -f {}-nm.mdp -o {}-nm.tpr".format(
                    self.name, self.name, self.name, self.name
                )
            )
        else:
            ...
            self.mol[shot].write("%s.gro" % self.name)
            self.warngmx(
                "grompp -c {}.gro -p {}.top -f {}-nm.mdp -o {}-nm.tpr".format(
                    self.name, self.name, self.name, self.name
                )
            )

        self.callgmx(f"mdrun -deffnm {self.name}-nm -nt 1 -mtx {self.name}-nm.mtx -v")
        self.callgmx(
            "g_nmeig -s %s-nm.tpr -f %s-nm.mtx -of %s-nm.xvg -v %s-nm.trr -last 10000 -xvg no"
            % (self.name, self.name, self.name, self.name)
        )
        self.callgmx(
            "trjconv -s {}-nm.tpr -f {}-nm.trr -o {}-nm.gro -ndec 9".format(
                self.name, self.name, self.name
            ),
            stdin="System",
        )
        ...
        calc_eigvals = np.array(
            [
                float(line.split()[1])
                for line in open("%s-nm.xvg" % self.name).readlines()
            ]
        )
    ...
    def generate_positions(self):
        ...
        self.callgmx(
            "mdrun -deffnm {} -nt 1 -rerunvsite -rerun {}-all.gro".format(
                self.name, self.name
            )
        )
        self.callgmx(
            "trjconv -f {}.trr -o {}-out.gro -ndec 9 -novel -noforce".format(
                self.name, self.name
            ),
            stdin="System",
        )
    ...
    def scd_persnap(self, ndx, timestep, final_frame):
        ...
        for snap in range(0, final_frame + 1):
            self.callgmx(
                "g_order -s %s-md.tpr -f %s-md.trr -n %s-%s.ndx -od %s-%s.xvg -b %i -e %i -xvg no"
                % (self.name, self.name, self.name, ndx, self.name, ndx, snap, snap)
            )
            Scd_snap = []
            for line in open(f"{self.name}-{ndx}.xvg"):
    ...
    def calc_scd(self, n_snap, timestep):
        ...
        self.callgmx(
            f"make_ndx -f {self.name}-md.tpr -o {self.name}-sn1.ndx",
            stdin="\n".join(sn1_ndx),
        )
        self.callgmx(
            f"make_ndx -f {self.name}-md.tpr -o {self.name}-sn2.ndx",
            stdin="\n".join(sn2_ndx),
        )
    ...
    def molecular_dynamics(
        ...
        **kwargs,
        ...
        md_opts = OrderedDict(
            [("integrator", "md"), ("nsteps", nsteps), ("dt", timestep / 1000)]
        )
        ...
        if temperature is not None:
            ref_t_str = " ".join(
                ["%f" % temperature] * len(mdp_opts.get("tc_grps", "System").split())
            )
        if pressure is not None:
            ref_p_str = " ".join(
                ["%f" % pressure] * len(mdp_opts.get("tc_grps", "System").split())
            )
        ...
        # Minimize the energy.
        if minimize:
            min_opts = OrderedDict(
                [("integrator", "steep"), ("emtol", 10.0), ("nsteps", 10000)]
            )
            if verbose:
        ...
        if nequil > 0:
            ...
            self.warngmx(
                "grompp -c {} -p {}.top -f {}-eq.mdp -o {}-eq.tpr".format(
                    gro1, self.name, self.name, self.name
                ),
                warnings=warnings,
                print_command=verbose,
            )
            self.callgmx(
                "mdrun -v -deffnm %s-eq -nt %i -stepout %i"
                % (self.name, threads, nsave),
                print_command=verbose,
        ...
        self.warngmx(
            "grompp -c {} -p {}.top -f {}-md.mdp -o {}-md.tpr".format(
                gro2, self.name, self.name, self.name
            ),
            warnings=warnings,
            print_command=verbose,
        )
        self.callgmx(
            "mdrun -v -deffnm %s-md -nt %i -stepout %i" % (self.name, threads, nsave),
            print_command=verbose,
            print_to_screen=verbose,
        )
        ...
        self.callgmx(
            "g_dipoles -s {}-md.tpr -f {}-md.trr -o {}-md-dip.xvg -xvg no".format(
                self.name, self.name, self.name
            ),
            stdin="System\n",
        )
        ...
        if bilayer:
            ...
            al_vars = ["Box-Y", "Box-X"]
            self.callgmx(
                "g_energy -f {}-md.edr -o {}-md-energy-xy.xvg -xvg no".format(
                    self.name, self.name
                ),
                stdin="\n".join(al_vars),
            )
        ...
        self.callgmx(
            "g_energy -f {}-md.edr -o {}-md-energy.xvg -xvg no".format(
                self.name, self.name
            ),
            stdin="\n".join(ekeep),
        )
...
class Liquid_GMX(Liquid):
    def __init__(self, options, tgt_opts, forcefield):
        ...
        self.set_option(
            tgt_opts, "liquid_coords", default="liquid.gro", forceprint=True
        )
        ...
        self.gas_engine_args = {
            "gmx_top": "%s.top" % os.path.splitext(self.gas_coords)[0],
    ...
    def npt_simulation(self, temperature, pressure, simnum):
        ...
        if self.goodstep and (temperature, pressure) in self.LfDict_New:
            self.LfDict[(temperature, pressure)] = self.LfDict_New[
        ...
        if (temperature, pressure) in self.LfDict:
            ...
            logger.info(
                "Copying previous iteration final geometry .gro file: {} to {}\n".format(
                    lfsrc, lfdest
                )
            )
            shutil.copy2(lfsrc, lfdest)
            self.nptfiles.append(lfdest)
        self.LfDict_New[(temperature, pressure)] = os.path.join(
            os.getcwd(), "liquid-md.gro"
        )
        super().npt_simulation(temperature, pressure, simnum)
...
class Lipid_GMX(Lipid):
    def __init__(self, options, tgt_opts, forcefield):
        ...
        self.nptfiles += [
            "%s.top" % os.path.splitext(f)[0] for f in [self.lipid_coords]
        ]
        # Scripts to be copied from the ForceBalance installation directory.
        self.scripts = ["rungmx.sh"]
        # Initialize the base class.
    ...
    def npt_simulation(self, temperature, pressure, simnum):
        ...
        if "n_ic" in self.RefData:
            ...
            if not PT_vals in self.lipid_mols_new:
                self.lipid_mols_new[PT_vals] = [
                    os.path.join(os.getcwd(), "lipid-md.gro")
                ]
            else:
                self.lipid_mols_new[PT_vals].append(
                    os.path.join(os.getcwd(), "lipid-md.gro")
                )
            # Run NPT simulation.
            super().npt_simulation(temperature, pressure, simnum)
