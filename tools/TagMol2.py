...
def get_equivalent_atoms(MyG):
    ...
    for A in PairPaths:
        ...
        for B in PairPaths[A]:
            if PairPaths[A][B] > Max:
                continue
            Walk[PairPaths[A][B]].append(GDict[B]["e"])
        for idx, elist in Walk.items():
            Walk[idx] = "".join(
                [
                    ("%s%i" % (k, elist.count(k)) if elist.count(k) > 1 else "%s" % k)
                    for k in sorted(set(elist))
                ]
            )
            # ef =
            # Walk[i] =
...
def charge_as_array(M2Mol, QMat):
    ...
    print(
        "Doing something stupid to make sure all of the charges add up to EXACTLY an integer."
    )
    ...
    while True:
        ...
        if M == 10000:
            raise Exception(
                "Tried 10,000 iterations of charge adjustment, I probably screwed up."
            )
