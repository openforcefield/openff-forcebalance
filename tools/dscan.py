...
parser.add_option(
    "--phi1",
    help="Quartet of atoms for 1D dihedral scan (or first quartet for 2D scan",
    type=int,
    nargs=4,
)
parser.add_option("--phi2", help="Second quartet of atoms for 2D", type=int, nargs=4)
parser.add_option(
    "--scan", help="Increment by which we wish to scan the dihedral angle", type=int
)
parser.add_option(
    "--thre",
    help="Scale the following clash radii: H-[NO] = 1.5, Hvy-Hvy = 2.4, H-Hvy = 2.0, H-H = 1.8",
    type=float,
    default=1.0,
)
    ...
    for grp1, grp2 in check_grps:
        for i1 in grp1:
            for i2 in grp2:
                ...
                if i12 in mol.bonds or i12 in one_three:
                    continue
                if e12 in ["HN", "HO", "FH"]:
                    thre = opts.thre * 1.5
                elif e12 == "HH":
...
def main():
    ...
    if len(xyzout) > 0:
        ...
        M.comms = commout
        M.write(
            os.path.splitext(opts.coords)[0] + "_out" + os.path.splitext(opts.coords)[1]
        )
    if len(xyzcsh) > 0:
        M.xyzs = xyzcsh
        M.comms = commcsh
        M.write(
            os.path.splitext(opts.coords)[0]
            + "_clash"
