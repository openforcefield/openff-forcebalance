...
from openff.forcebalance.nifty import _exec, printcool
...
def getCentroid(r_rmsd, beta=1.0):
    ...
    """
    if len(r_rmsd) > 1:
        return np.exp(-beta * r_rmsd / r_rmsd.std()).sum(axis=1).argmax()
    else:
        return 0
...
class Reopt:
    ...
    def parseFBInput(self):
        ...
        ff_path = os.path.join(
            "result", os.path.splitext(self.options["input_file"])[0]
        )
    ...
    def minGrids(self):
        ...
        cwd = os.getcwd()
        if not os.path.isdir(f"{self.base_tmp}/Cluster"):
            os.makedirs(f"{self.base_tmp}/Cluster")
        for k in self.unique_res:
            ...
            for i in range(len(self.unique_res[k])):
                ...
                if not os.path.isdir(f"{self.opt_name}"):
                    os.makedirs(f"{self.opt_name}")
                os.chdir(f"{self.opt_name}")
                for struct in range(len(self.unique_res[k][i].engine.mol)):
                    if not os.path.isdir(f"{struct}"):
                        os.makedirs(f"{struct}")
                    os.chdir(f"{struct}")
                    engine_files = os.listdir(f"{scr}")
                    [
                        os.symlink(f"{scr}/{z}", z)
                        for z in engine_files
                        if not os.path.isdir(f"{scr}/{z}")
                    ]
                    os.symlink(
                        os.path.join(
                            self.root,
                    ...
                    geom.write(f"{self.min_file_name}")
                    os.chdir(os.path.join(scr, self.opt_name))
                _exec(
                    "find . -name {} | sort | xargs cat > mm_opt.gro".format(
                        self.min_file_name
                    )
                )
                ...
                if collect is None:
                    ...
                    collect.append(mol)
                os.chdir(cwd)
            os.chdir(f"{self.base_tmp}/Cluster")
        ...
        for k in self.unique_res:
            ...
            if not os.path.isfile("centroids.gro"):
                ...
                if self.cluster_inds == "heavy":
                    self.indices = [i for i in range(len(elems)) if elems[i] != "H"]
                elif self.cluster_inds == "all":
                    self.indices = [i for i in range(len(elems))]
                else:
                    raise Exception(
                        "{} is an invalid option for default_cluster_indices.".format(
                            self.default_cluster_indices
                        )
                    )
                ...
                clusters = hierarchy.fcluster(
                    link, t=self.clust_dist, criterion="distance"
                )
        ...
        for k in self.unique_res:
            ...
            for struct in range(len(self.unique_res[k][0].engine.mol)):
                if not os.path.isdir(f"{struct}"):
                    os.makedirs(f"{struct}")
                os.chdir(f"{struct}")
                if not os.path.isfile(f"{self.min_file_name}"):
                    for f in os.listdir("."):
                        os.remove(f)
                    engine_files = os.listdir(f"{scr}")
                    [
                    ...
                    geom.write(f"{self.min_file_name}")
                os.chdir(
                    "{}".format(os.path.join(self.root, self.base_tmp, "Cluster", k))
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="For use in conjunction with ForceBalance to identify spurious minima in the MM potential energy surface. The outputs from this program can then be added as ForceBalance targets for another optimization cycle.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument(
        "fbinput",
        type=str,
        help="The name of the ForceBalance optimization input file.",
    )
    parser.add_argument(
        "--default_cluster_indices",
        type=str,
        default="heavy",
        help="The default clustering code will only pick out heavy atoms. Write 'all' if you want all indices to be used.",
    )
    parser.add_argument(
        "--cluster_dist",
        type=float,
        default=0.1,
        help="Distance criterion to be used for forming the hierarchical clusters",
    )
