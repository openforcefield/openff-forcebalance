...
try:
    ...
    raise Exception(
        "To make sure we find the gaff.dat file, please make sure the $AMBERHOME environment variable is set."
    )
...
for line in open(DataFile).readlines():
    ...
    if isfloat(sline[1]) and isfloat(sline[2]):
        ...
        dline = dline.replace(
            "Head Sp2 C that connect two rings in biphenyl sys.", "bridgehead C"
        )
        dline = dline.replace(
            "H bonded to aliphatic carbon without electrwd. group", "H-C (aliph) 0 EWG"
        )
        dline = dline.replace(
            "H bonded to aliphatic carbon with 1 electrwd. group", "H-C (aliph) 1 EWG"
        )
        dline = dline.replace(
            "H bonded to aliphatic carbon with 2 electrwd. group", "H-C (aliph) 2 EWG"
        )
        dline = dline.replace(
            "H bonded to aliphatic carbon with 3 electrwd. group", "H-C (aliph) 3 EWG"
        )
        dline = dline.replace(
            "H bonded to C next to positively charged group", "H-C (positive), 3 EWG"
        )
        ...
        Atypes[sline[0]] = r"\ ".join(dline.split())
...
for line in os.popen("awk '/ATOM/,/BOND/' %s" % sys.argv[1]):
    ...
    if len(sline) == 9:
        try:
            Descriptions.append(r"\ \ \ \[%s\]\ " % sline[5] + Atypes[sline[5]])
        except:
...
for xyz, desc in zip(XYZs, Descriptions):
    print(
        "graphics top text {% .3f % .3f % .3f} %s size 0.4"
        % (xyz[0], xyz[1], xyz[2], desc),
        file=OutVMD,
    )
...
if RunVMD:
    ...
    print(
        "Finished analyzing mol2 file.  Now run vmd with the following command: vmd -e %s"
        % vmdfnm
    )
