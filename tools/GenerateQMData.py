...
def drive_msms(xyz, radii, density):
    with open(os.path.join("msms_input.p"), "w") as f:
        lp_dump((xyz, radii, density), f)
        ...
        for i in range(M.ns):
            ...
            if os.path.exists(dnm):
                ...
                if os.path.exists("qchem.out"):
                    ...
                    raise Exception(
                        "The output file %s doesn't exist."
                        % os.path.abspath("qchem.out")
                    )
                os.chdir("..")
            else:
                raise Exception(
                    "The subdirectory %s doesn't exist." % os.path.abspath(dnm)
                )
        os.chdir("..")
        return Result
...
def gather_generations():
    ...
    if A1.shape != A2.shape:
        ...
        raise Exception(
            "shots.gro and qdata.txt appear to contain different xyz coordinates"
        )
...
def main():
    ...
    if len(sys.argv) != 2:
        print(
            "Please call this program with only one argument - the name of the input file."
        )
        sys.exit(1)
