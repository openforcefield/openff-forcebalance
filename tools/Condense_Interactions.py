...
for line in ffdata:
    ...
    if sec == "atoms" and re.match("^ *[0-9]", line):
        ...
        if key in idict and idict[key] != val:
            skips.append(strip)
            sys.stderr.write(
                "Key %s already exists in idict with a different value (%s -> %s)\n"
                % (key, idict[key], val)
            )
        else:
            idict[key] = val
...
for line in ffdata:
    ...
    if sec == "moleculetype" and Insert:
        ...
        for key, val in bondtypes.items():
            print(
                "".join(["%5s" % i for i in key.split(",")])
                + "".join(["%12s" % i for i in val.split(",")])
            )
        print()
        print("[ angletypes ]")
        ...
        for key, val in dihedraltypes.items():
            print(
                "".join(["%5s" % i for i in key.split("_")[0].split(",")])
                + "".join(["%12s" % i for i in val.split(",")])
            )
        print()
